<?php
    ...
    $sqlpo = $koneksi->query("SELECT namapo FROM poproduk INNER JOIN bukapo ON bukapo.idpoproduk = poproduk.idpoproduk WHERE poproduk.idpoproduk = '$idpoproduk'");
...
<body>
    <nav class="navbar bg-body-tertiary py-3">
        <div class="container-fluid row">
            <div class="col-2 text-center">
                <a class="navbar-brand" href="datapokonin.php?id=<?= $idpoproduk ?>&invoice=<?= $invoice ?>"><i class="bi bi-chevron-left"></i></a>
            </div>
    ...
    <div class="container mt-3">
        ...
        <?php
            $query = $koneksi->query("SELECT pomitra.invoice, pomitra.total, pomitra.status, podropship.dropship FROM podropship LEFT JOIN pomitra ON pomitra.invoice = podropship.invoice WHERE pomitra.invoice = '$invoice'");
            while ($datainv = $query->fetch_assoc()) {
                $harga += $datainv['total'];
                $biayads = $datainv['dropship'];
                $status = $datainv['status'];
            }
            $persen = 35;
            $diskon = $persen / 100 * $harga;
            $grandtotal = $harga - $diskon;
            if ($status == "Belum DP") {
                $ttotal = 50/100 * $grandtotal;
            } elseif ($status == "Sudah Payment DP 1") {
                $ttotal = 30/100 * $grandtotal;
            } else {
                $ttotal = 20/100 * $grandtotal;
            }
        ?>
            ...
            <div class="card-body">
                <form method="post" class="row" enctype="multipart/form-data">
                    <div class="col-md-5">
                        ...
                        <hr />
                        <div class="form-group mb-2">
                            <p class="fw-medium text-muted mb-0" style="font-size: .725rem"><span class="text-danger">*</span> Jumlah transfer tidak termasuk ongkir</p>
                            <label for="jumlah-transfer" class="form-label mb-1">Jumlah Transfer</label>
                            <input type="text" name="jumlah_transfer" class="form-control form-control-sm bg-secondary bg-opacity-10 pe-none" id="jumlah-transfer" value="Rp. <?= number_format(num: $ttotal) ?>" readonly>
                        </div>
                        <div class="form-group mb-2">
                            <label for="jenis-pembayaran" class="form-label mb-1">Jenis Pembayaran</label>
                            <?php if ($status == "Belum DP") : ?>
                                <input type="text" name="jenis_pembayaran" class="form-control form-control-sm bg-secondary bg-opacity-10 pe-none" id="jenis-pembayaran" value="Payment DP 1 (50%)" readonly>
                            <?php elseif ($status == "Sudah Payment DP 1") : ?>
                                <input type="text" name="jenis_pembayaran" class="form-control form-control-sm bg-secondary bg-opacity-10 pe-none" id="jenis-pembayaran" value="Payment DP 2 (30%)" readonly>
                            <?php elseif ($status == "Sudah Payment DP 2") : ?>
            ...
            try {
                ...
                if ($foto <> '') {
                    ...
                    if (!in_array($ekstensiGambar, $ekstensiGambarValid)) {
                        ...
                        echo "<script>location='popembayarankonin.php?id=$idpoproduk&invoice=$invoice';</script>";
                        return false;
                    }
                ...
                if (move_uploaded_file($tmp, 'bukti/' . $namaFileBaru)) {
                    try {                            
                        if ($status == "Belum DP") {
                            ...
                            $status_pembayaran = "Payment DP 1";
                        } elseif ($status == "Sudah Payment DP 1") {
                            $jenis_payment = "Payment 2";
                            $status_pembayaran = "Payment DP 2";
                        } elseif ($status == "Sudah Payment DP 2") {
                            // $jenis_payment = "Payment 3";
                            // $status_pembayaran = "Payment DP 3";
                            $jenis_payment = "Lunas";
                            $status_pembayaran = "Pelunasan";
                        } elseif ($status == "Payment DP 3") {
                        }
                        ...
                        $sqlins = $koneksi->query("INSERT INTO popembayaran VALUES
                                                    (
                                                        ...
                                                        NULL, NULL, NULL,
                                                        '$metodebayar', '$status_pembayaran', 'PO Konin 2025',
                        ...
                        if ($sqlins) {
                            ...
                            echo "
                                <script>
                                    ...
                                    location='popembayarankonin.php?id=$idpoproduk&invoice=$invoice'
                                </script>
                            ";
                        }
                    } catch (Exception $e) {
