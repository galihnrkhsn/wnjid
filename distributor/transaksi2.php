...
<div class="container">
        ...
        <form method="post">
            <input type="text" name="invoice" placeholder="  tulis nomor invoice" class="form-control"><button type="submit" class="btn btn-warning" name="cari">Cari</button><br><br>
            </form>
        <p>Order DB Zinnia</p>
				...
				<table class="table table-bordered">
					...
					if(isset($_POST["cari"])){
    					...
    					$sql = mysqli_query($koneksi, "SELECT DISTINCT * FROM `ordermitra` where invoice='$invoice' GROUP BY invoice");
					...
					$idmitra=$_SESSION['admin_mitra']['idadmin'];
					// Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					$sql = mysqli_query($koneksi, "SELECT DISTINCT * FROM `ordermitra` inner join admin_mitra on admin_mitra.idadmin=ordermitra.idmitra where ordermitra.idmitra='$idmitra'  GROUP BY invoice ORDER BY idorder DESC limit 20 ");
		...
		<!--		   <p>Order Mitra Zinnia</p>
				...
				<table class="table table-bordered">
					...
					$idmitra=$_SESSION['admin_mitra']['idadmin'];
					// Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					$sql = mysqli_query($koneksi, "SELECT DISTINCT * FROM `ordermitra` inner join admin_mitra on admin_mitra.idadmin=ordermitra.idmitra GROUP BY invoice ORDER BY idorder DESC limit 20 ");
					...
					?>
						<tr>
							...
							<td class="align-middle"><?php echo $data['status']; ?></td>
							<td class="align-middle"><a href="detailorder.php?id=<?php echo $data['invoice']; ?>"><?php echo $data['invoice']; ?></a></td>
							<td class="align-middle"><form method="post"><input type="hidden" name="invoice" value=<?php echo $data['invoice']; ?>><button type="submit" class="btn btn-primary" name="approve">Approve</button></form></td>
