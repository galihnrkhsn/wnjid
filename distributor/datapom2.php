	  ...
	  <p align="left">No Invoice  : <?php echo $invoice; ?> </p>
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					<tr>
					 <th>No</th>
					 <?php if ($idpoproduk === '237' or $idpoproduk === '247') : ?>
              <th width="300">Nama Barang</th>
              <th width="300">Jenis PO</th>
    				 <?php elseif ($idpoproduk === '234') : ?>	 
    			         <th>Barang</th>
    				 ...
    				 <?php endif; ?>
					 <!-- <th>Font Teks</th> -->
					 <?php if($idpoproduk === '234') : ?>
    					 <th width="200">Pack</th>
            <?php elseif($idpoproduk === '237' or $idpoproduk === '247' or $idpoproduk == '261') :?>
              <th width="50">Qty</th>
              <th>Font</th>
					 <?php else : ?>
    					 <th width="200">Qty</th>
            ...
            pomitra.is_custom
                ...
                <?php if ($idpoproduk === '237' or $idpoproduk === '247' or $idpoproduk == '261') : ?>
                  <td><?= $data['jumlah'] ?></td>
                  <td><?= $data['font']; ?></td>
                <?php else : ?>
                  <td class="align-middle">
                ...
                <?php endif; ?>
                    <?php if($idpoproduk == '261') : ?>
					     	...
					     	<td class="align-middle"><?php echo $data['total']*$data['jumlah']; ?></td>
                    <?php else : ?>
                    ...
                    <?php endif; ?>
   				     <?php
              $sum+= $data['jumlah'];
              $diskon_bayar = $sum * 5000;
              if ($idpoproduk == '261') {
                $jumlah += $data['jumlah']*$data['total'];
              } elseif ($idpoproduk == '270') {
                 $a += $data['jumlah']*$data['harga'];
                 $jumlah = $a - $diskon_bayar;
              } else {
...
<table style="float: right;width: 100%">
 <tbody  style="float: right;">
    ...
    <?php if($idpoproduk == '261' & $idpoproduk == '270') : ?>
    
    <tr>
        <th>Potongan</th>
        <td>:</td>
        <td>Rp. <?= number_format($diskon_bayar); ?></td>
    </tr>
...
<?php if($tampilkan['idpoproduk'] == '260' or $tampilkan['idpoproduk'] == '269') : ?>
    <button type="submit" class="btn btn-success btn-sm" name="cari" id="linkmiki<?= $tampilkan['idbpo']; ?>">
...
<?php if($tampilkan['idpoproduk'] == '261') : ?>
    <button type="submit" class="btn btn-success btn-sm" name="cari" id="linkmiki<?= $tampilkan['idbpo']; ?>">
        <a style="color:white" href="ubahpolegging.php?id=<?= $idpoproduk;?>&invoice=<?= $invoice;?>">Ubah Variant</a>
    </button>
    <button type="submit" class="btn btn-info btn-sm" name="cari" id="linkmiki<?= $tampilkan['idbpo']; ?>">
        <a style="color:white" href="formpocustomlegging2.php?id=<?= $idpoproduk;?>">Tambah Variant</a>
    </button>
<!--<?//php elseif($tampilkan['idpoproduk'] == '269') : ?>-->
<!--    <button type="submit" class="btn btn-info btn-sm" name="cari" id="linkmiki<?= $tampilkan['idbpo']; ?>">-->
<!--        <a style="color:white" href="formpomatari.php?id=<?= $idpoproduk;?>&invoice=<?= $invoice ?>">Tambah Variant</a>-->
<!--    </button>-->
<?php endif; ?>
        ...
        if(isset($_POST['btn_isCustom'])) {
          ...
          if ($valueIsCustom == 0) {
            ...
            if ($updateQuery) {
              ...
              echo "<script>window.location.href='https://wnj.web.id/distributor/datapom2.php?id=".$_GET['id']."&invoice=".$_GET['invoice']."';</script>";
            } else {
