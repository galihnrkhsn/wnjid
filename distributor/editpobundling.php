        ...
        if (isset($_POST['ubah_data'])) {
            try {
                ...
                foreach ($data_rows as $row) {
                    ...
                    if ($bukapo['jenis_po'] === 'PO Custom Stok') {
                        $stok_lama      = (int)$row['jumlah'];
                        $selisih        = $qty - $stok_lama;
                        $stokQuery      = $koneksi->query("SELECT stok, namakategori FROM pokategori WHERE idpo = '$idpo' FOR UPDATE");
                        $stokData       = $stokQuery->fetch_assoc();
                        $stok_sekarang  = (int)$stokData['stok'];
                        if ($selisih > 0 && $selisih > $stok_sekarang) {
                            throw new Exception("Stok tidak mencukupi untuk variant:" . $stokData['namakategori']);
                        }
                        ...
                        ");
                        if (!$updateStok) {
                            throw new Exception("Gagal memperbarui data: $namakategori");
                        }
                    }
                }

                $koneksi->commit();
                if ($bukapo['jenis_po'] == 'PO Bundling 2' || $bukapo['jenis_po'] == 'PO Bundling 5' || $bukapo['jenis_po'] == 'PO Custom Stok') {
                    // Jika semua berhasil, redirect
