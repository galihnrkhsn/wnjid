...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_custom') {
        ...
        $template_val = trim($_POST['custom_val']);
        $ok = $koneksi->query("
            UPDATE pomitra SET template = '$template_val'
            WHERE idpomitra = '$idpomitra' AND idmitra = '$idadmin'
        ");
        $flash = $ok ? ['type'=>'success','msg'=>'Template berhasil diperbarui.']
                     : ['type'=>'error',  'msg'=>'Gagal memperbarui template.'];
        header("Location: ubahpo_custom_template.php?id=$idpoproduk&invoice=$invoice&flash=".urlencode($flash['msg'])."&ftype=".$flash['type']);
        exit();
    }
    ...
    if ($action === 'insert_new') {
        ...
        $flash = $inserted > 0
            ...
            : ['type'=>'error',  'msg'=>'Tidak ada baris yang ditambahkan. Pastikan Template diisi.'];
        header("Location: ubahpo_custom_template.php?id=$idpoproduk&invoice=$invoice&flash=".urlencode($flash['msg'])."&ftype=".$flash['type']);
        exit();
    }
}
            ...
            <div class="exist-row" style="animation-delay: <?= $idx * 0.04 ?>s">
                <div class="er-left">
                    ...
                    <form method="post" class="custom-edit-row">
                        ...
                        <span style="font-size:.75rem;color:var(--muted);white-space:nowrap">Template:</span>
                        <select  class="form-control" name="custom_val" class="inline-custom">
                            <option value="" disabled>-- Pilih --</option>
                            <?php foreach (range(1, 27) as $n): ?>
                                <?php $opt = 'LM-' . str_pad($n, 2, '0', STR_PAD_LEFT); ?>
                                <option value="<?= $opt ?>" <?= ($ex['template'] ?? '') === $opt ? 'selected' : '' ?>>
                                    <?= $opt ?>
                                </option>
                            <?php endforeach; ?>

                            <?php foreach (range(1, 18) as $n): ?>
                                <?php $opt = 'LB-' . str_pad($n, 2, '0', STR_PAD_LEFT); ?>
                                <option value="<?= $opt ?>" <?= ($ex['template'] ?? '') === $opt ? 'selected' : '' ?>>
                                    <?= $opt ?>
                                </option>
                            <?php endforeach; ?>
...
function generateCustomInputs(idpodetail, qty) {
    ...
    // Template options LM-01 s/d LM-20
    const templateOptions = [
        // Logo Mobil (LM-01 s/d LM-27)
        ...Array.from({ length: 27 }, (_, i) => {
            const num = String(i + 1).padStart(2, '0');
            return `LM-${num}`;
        }),

        // Logo Bola (LB-01 s/d LB-18)
    ...
    const existing = [];
    grid.querySelectorAll('select[name="customs[]"]').forEach(s => existing.push(s.value));

    grid.innerHTML = '';
    for (let i = 0; i < qty; i++) {
        const optionsHTML = templateOptions
            .map(opt => `<option value="${opt}" ${existing[i] === opt ? 'selected' : ''}>${opt}</option>`)
            .join('');
        ...
        item.innerHTML = `
            ...
            <select class="form-control" name="customs[]" required>
...
function confirmDelete(idpomitra, variant, custom) {
    ...
    document.getElementById('deleteModalDesc').textContent =
        `Hapus baris "${variant}"${custom ? ' (template: ' + custom + ')' : ''}?`;
