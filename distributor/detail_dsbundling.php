...
<body>
    ...
    <div class="container mt-5">
        ...
        <? 
            $invocenya  = $data_ds['invoice'];
            $queryPay   = $koneksi->query("SELECT status AS statusnya FROM pomitra WHERE invoice = '$invocenya'");
            $dataPay    = $queryPay->fetch_assoc();
            ...
            if (isset($_POST["insert-variant"])) {
                ...
                try {
                    foreach ($_POST['hiddenProductInput'] as $key => $index) {
                        ...
                        if ($jumlah > 0) {
                             // Cek stok yang tersedia untuk varian
                            $stokQuery      = $koneksi->query("SELECT stok FROM pokategori INNER JOIN podetail ON pokategori.idpo = podetail.idpo WHERE podetail.idpodetail = '$variant'");
                            $stokData       = $stokQuery->fetch_assoc();
                            $stokTersedia   = $stokData['stok'];

                            try {
                                if ($jumlah > $stokTersedia) {
                                    throw new Exception("Jumlah yang dimasukkan melebihi stok yang tersedia ($stokTersedia).");
                                }
                            } catch (Exception $e) {
                                $koneksi->rollback();
                                echo $e->getMessage();
                                echo "<script>alert('Jumlah yang dimasukkan melebihi stok yang tersedia ($stokTersedia).')</script>";
                            ...
                            if ($checkQuery) {
                                if ($checkQuery->num_rows > 0) {
                                    ...
                                    $updateStok     = $koneksi->query("UPDATE pokategori SET stok = stok - '$jumlah' WHERE idpo = '$idpo");

                                    if ($updatePomitra && $updateStok) {
                                        ...
                                        if (!$insertPods) {
                                            ...
                                            throw new Exception("Gagal Update Stok Produk: " . $koneksi->error);
                                        }
                                    } else {
                                    ...
                                    (null, '$idadmin', '$idpoproduk', '$idpo', '$variant', '$jumlah', '$total', '$invoice', 'Belum DP', NOW(), '$waktu')");
                    ...
                    if ($processed) {
                        if ($idpoproduk == '271') {
                            ...
                            echo "<script>alert('data berhasil dikirim');</script>";
                            echo "<script>location='datapo?id=$idpoproduk&invoice=$invoice';</script>";
                        }
                    }
                } catch (Exception $e) {
                    ...
                    echo "<script>location='detailpo?id=$idpoproduk';</script>";
                }
            }
    ...
    <script type="text/javascript">
            ...
            variantForm.innerHTML = `
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        <select name="variant[]" class="form-control form-control-sm" id="variantSelect_${variantFormCount}" onchange="updateHiddenProductInput(this, ${variantFormCount})">
                            ...
                            <?php
                                $ambil = $koneksi->query("SELECT DISTINCT podetail.variant, podetail.idpodetail, pomitra.custom, pokategori.stok
                                                            FROM poproduk 
                                ...
                                if ($ambil) {
                                    while ($data = $ambil->fetch_assoc()) {
                                        $custom = $data['custom'];
                                        ?>
                                        <option value="<?= $data['idpodetail']; ?>" data-stok="<?= $data['stok']; ?>">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control form-control-sm" id="jumlahInput_${variantFormCount}" placeholder="Masukan Jumlah" name="jumlah[]" required oninput="validateQuantity(this, ${variantFormCount})">
                    </div>
                </div>
            `;
        ...
        function updateHiddenProductInput(selectElement, index) {
            ...
            document.getElementById(`jumlahInput_${index}`).max = stok; // Set max value to stok
        }

        function validateQuantity(inputElement, index) {
            const maxStok = inputElement.max;
            if (parseInt(inputElement.value) > parseInt(maxStok)) {
                alert('Jumlah tidak boleh lebih dari stok yang tersedia.');
                inputElement.value = maxStok; // Set the value to the max available stock
            }
        }
    </script>
