...
<!--================ NAVBARU END =================-->
  ...
  <div class="container panel panel-default">
    ...
    <input type="text" class="form-control" name="tlppengirim" required>
    </div>    
    ...
    <label width="1"><font color="grey">note: tidak untuk memakai tanda kutip dan simbol-simbol lainnya</font></label>
    </div>
   ...
   <center><button type="submit" class="btn btn-primary" name="kirim">Next</button></center>
  </form>
</body>
...
include "koneksi.php";
	...
	if(isset($_POST['kirim'])){
	 ...
	 $sql = mysqli_query($koneksi, "SELECT iddropship FROM podropship_konin order by iddropship desc limit 1");
			$data = mysqli_fetch_array($sql);
			$no=$data['iddropship'];
			$ab=1;
			$nobaru=$no+$ab;
	 
	  $query = "insert into podropship_konin (iddropship,idadmin,idpoproduk,invoice,namapengirim,tlppengirim,namapenerima,tlppenerima,alamatpenerima,keterangan,ekspedisi) values
	  (null,'$idadmin','28','KND$idadmin-$nobaru','$namapengirim','$tlppengirim','$namapenerima','$tlppenerima','$alamatpenerima','$keterangan','$ekspedisi')";
            $sql = mysqli_query( $koneksi, $query);  
            
            if ( $sql == TRUE ){
					echo "<script>alert('Berhasil Disimpan, Silahkan input PO yang diinginkan');</script>";
						echo "<script>location='formpo_konin.php?id=33&inv=KND$idadmin-$nobaru'</script>";
				}else{
					echo "<script>alert('Gagal Simpan, Coba Lagi');</script>";
						echo "<script>location='formdropship_konin.php'</script>";
				}
	}				
