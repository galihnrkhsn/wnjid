...
<!-- <label>Contoh Brooch : </label><br> -->
<!--                   <img src="img/broochrg.jpg" width="50%">
                  <br><br>
...
if($sisa['stok']>=1){
  if (preg_match("/^\S{1,}$/", $namanya)) {
      $sqlinsert = $koneksi->query("insert into pomitra (idpomitra,idmitra,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl) values
      (null,'$idadmin','$idpoproduk','$idpo','$idpodetail[$x]','$jumlah[$x]','$namanya','$total',
      'B$idpoproduk-$idadmin-$nobaru','Belum DP',NOW())");
...
if ($sqlstok) {
  echo "<script>location='datapom.php?id=$idpoproduk&invoice=B$idpoproduk-$idadmin-$nobaru';</script>";  
}
...
if($sisa['stok']<1){
  ...
  echo "<script>alert('Stok kami untuk Nama Custom ($namanya) tidak mencukupi, silahkan revisi pesanan anda sesuaikan dengan stok');</script>";
  if ($sqlstok) {
  echo "<script>location='datapom.php?id=$idpoproduk&invoice=B$idpoproduk-$idadmin-$nobaru';</script>";  
}
  echo "<script>location='formpobrooch_custom.php';</script>";
}
