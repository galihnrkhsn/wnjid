        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <div class="mb-2 bg-danger-subtle px-3 py-2 mx-2 rounded" role="alert">
                    <p class="m-0 fw-semibold" style="font-size: small">Ini Ubah Pre-Order Bundling, barang yang di pilih harus kelipatan 5!</p>
                </div>
    ...
    <?php
        if (isset($_POST['kirim'])) {
            try {
                ...
                // Input arrays dari form
                $idpodetail = $_POST['idpodetail'];   // array
                $idpomitra  = $_POST['idpomitra'];    // array
                $qty_inputs = $_POST['qty'];          // array

                // Pastikan koneksi mysqli ada di $koneksi
                if (!$koneksi) {
                    throw new Exception("Koneksi database belum diinisialisasi.");
                }

                // Invoice (sama seperti kode semula)
                ...
                // Validasi kelipatan 5 pada total_item
                if ($total_item % 5 !== 0) {
                    echo "
                        <script>
                ...
                // Mulai transaksi agar update pomitra + pokategori konsisten
                ...
                for ($x = 0; $x < count($idpodetail); $x++) {
                    ...
                    // Ambil data podetail + pokategori
                    $q = $koneksi->query("
                    ...
                    // Ambil qty lama dari table pomitra (jika ada)
                    $q2 = $koneksi->query("
                        SELECT jumlah 
                        FROM pomitra 
                    ...
                    // Hitung selisih (positif = perlu kurangi stok; negatif = perlu tambah stok)
                    $selisih = $qty_baru - $qty_lama;

                    // Jika penambahan qty, cek stok cukup
                    if ($selisih > 0) {
                        if ($selisih > $stok_db) {
                            $koneksi->rollback();
                            echo "
                    ...
                    // Update pomitra: jika record ada -> update, jika tidak ada -> insert (opsional)
                    if ($row_pomitra) {
                        $upd = $koneksi->query("
                            UPDATE pomitra
                        ...
                        // Jika tidak ada record pomitra, insert baru (jika qty_baru > 0)
                        if ($qty_baru > 0) {
                            $ins = $koneksi->query("
                                INSERT INTO pomitra (idpomitra, idmitra, idpoproduk, idpo, idpodetail, jumlah, custom, total, invoice, status, tgl, waktu)
                                VALUES (NULL, '$iddb', '$idpoproduk', '{$data['idpo_db']}', '$idpodetail_item', $qty_baru, '$jenis_po', $harga_total, '$invoice', 'Belum DP', NOW(), '$waktu')
                            ");
                            if (!$ins) {
                                $koneksi->rollback();
                    ...
                    // Update stok di pokategori berdasarkan selisih
                    if ($selisih > 0) {
                        // kurangi stok
                    ...
                    // (Opsional) Jika qty_baru == 0 dan kamu ingin menghapus row pomitra, uncomment:
                    // if ($qty_baru == 0 && $row_pomitra) {
                    //     $del = $koneksi->query("DELETE FROM pomitra WHERE idpomitra = '$idpomitra_item'");
                    //     if (!$del) { $koneksi->rollback(); throw new Exception("Gagal delete pomitra: ".$koneksi->error); }
                    // }
                } // end loop
                ...
                // Redirect sukses setelah semua selesai
                echo "
                    <script>
                ...
                // Jika terjadi error, rollback kalau transaksi berjalan
                if (isset($koneksi) && $koneksi->connect_errno == 0) {
                    $koneksi->rollback();
                }
                // Debug / tampilkan pesan
