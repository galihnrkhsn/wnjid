...
<br> 
 ...
 <?php
 $idponya = $idponya-1;
for ($x = 0; $x <  $jumlahidponya; $x++) {

$idponya = $idponya+1;
  $query = "SELECT * FROM pokategori WHERE idpo='$idponya'";
  $sqlkategori = mysqli_query($koneksi, $query);  
  $datakategori = mysqli_fetch_array($sqlkategori); 
?>              
<p>Kategori <?php echo $datakategori['namakategori']; ?></p>          
    <select class="form-control " name="idpodetail[]" required>
     <option>
        Pilih Seri Sarung
            ...
            $total = 0;
            $index = 0;
            $sql = "SELECT * FROM poproduk inner join pokategori inner join podetail on poproduk.idpoproduk=pokategori.idpoproduk and pokategori.idpo=podetail.idpo where poproduk.idpoproduk='$idpoproduk' and pokategori.idpo = '$idponya' order by pokategori.idpo desc";
...
for($x=0;$x<$jumlah_dipilih;$x++){
  ...
  (null,'$idadmin','$idpoproduk','$idpo','$idpodetail[$x]','1','$harga','S$idpoproduk-$idadmin','Belum DP',NOW())");   
