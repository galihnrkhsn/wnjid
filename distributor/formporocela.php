        ...
        <form method="POST">
          <div class="row">
            <?php
              ...
              $index = 0;
              $number = 1;
              $sql = "SELECT * FROM poproduk inner join pokategori inner join podetail on poproduk.idpoproduk=pokategori.idpoproduk and pokategori.idpo=podetail.idpo where poproduk.idpoproduk='$idpoproduk' and podetail.variant LIKE '%Rocela%' order by podetail.idpodetail asc";
    ...
    if (isset($_POST["save"])) {
        ...
        if (!empty($queries)) {
            ...
            if (mysqli_multi_query($koneksi, $queries_string)) {
                echo "<script>location='datapocustom.php?id=$idpoproduk&invoice=$invoice&idadmin=$idadmin'; </script>";
            } else {
                echo "Gagal!";
            }
        } else {
