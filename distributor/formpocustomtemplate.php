...
$ketentuan_list = $po_config[$idpoproduk]['ketentuan'] ?? [
    ...
    '<strong style="color:red;">LB = Logo Bola (18 Pilihan Template)</strong>',
    '<strong style="color:red;">LM = Logo Mobil (27 Pilihan Template)</strong>',
];
...
if (isset($_POST['save'])) {
    try {
        ...
        if (isset($_GET['invoice'])) {
            ...
            $invoice = 'DCT' . $idpoproduk . '-' . $idadmin . generateAngkaAcak(5);
        }
        ...
        foreach ($idpodetail as $x => $idDetailNow) {
            ...
            if ($jumlahCustom != $qty)
                throw new Exception("Jumlah Template untuk variant {$detail['variant']} harus $qty buah.");

            foreach ($customArr as $template) {
                if (empty($template)) throw new Exception("Terdapat Template kosong pada variant {$detail['variant']}.");

                $ok = $koneksi->query("
                    INSERT INTO pomitra
                        (idpomitra, idmitra, idpoproduk, idpo, idpodetail, jumlah, total, template, invoice, status, tgl, waktu)
                    VALUES
                        (NULL, '$idadmin', '$idpoproduk', '$idpo', '$idDetailNow',
                         '1', '$harga', '$template', '$invoice', 'Belum DP', NOW(), '$waktu')
                ");
                ...
                <div class="qty-row">
                    ...
                    <div class="qty-stepper">
                        ...
                        <input type="number"
                               ...
                               class="qty-input"
                               oninput="generateTemplate(<?= $row['idpodetail'] ?>, this.value)">
                        <button type="button" onclick="stepQty(<?= $row['idpodetail'] ?>, 1)">+</button>
                    </div>
                </div>
...
function stepQty(id, delta) {
    ...
    inp.value = val;
    generateTemplate(id, val);
    updateTotal();
}

// ── Generate template inputs ──────────────────────────────────────────────────
function generateTemplate(idpodetail, qty) {
    qty = parseInt(qty) || 0;
    ...
    const prev = [];
    grid.querySelectorAll('select').forEach(s => prev.push(s.value));

    grid.innerHTML = '';
    for (let i = 0; i < qty; i++) {
        const optionsHTML = templateOptions
            .map(opt => `<option value="${opt}" ${prev[i] === opt ? 'selected' : ''}>${opt}</option>`)
            .join('');
        ...
        div.innerHTML = `
            ...
            <select class="form-control" name="custom[${idpodetail}][]" required>
                <option value="" ${!prev[i] ? 'selected' : ''} disabled>-- Pilih Template --</option>
                ${optionsHTML}
            </select>
        `;
