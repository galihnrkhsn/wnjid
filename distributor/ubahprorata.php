...
<input type="text" name="namaproduk">  <button type="submit" class="btn btn-primary" name="cari">Cari</button>  <button type="submit" class="btn btn-primary" name="tampil">Tampil Semua</button></form></pre>-->  
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					FROM poproduk inner JOIN pokategori inner join podetail inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail WHERE pomitra.idmitra='$idadmin' and pomitra.idpoproduk='$idpoproduk' order by podetail.idpodetail asc");
                            ...
                            if(isset($_POST["edit"])){
                                            ...
                                            //echo number_format($total, 2);
                                              ...
                                              //$tot=0;
        					                   $koneksi->query("UPDATE pomitra set jumlah='$jmlh', total='$total' where idpomitra='$idpomitra';");
		                                        ...
		                                        echo "<script>location='ubahprorata.php?id=$idpoproduk';</script>";
        					                     }
        					   elseif(isset($_POST["hapus"])){
					               ...
					               $koneksi->query("delete from pomitra where idpomitra='$idpomitra';");
        		                   	echo "<script>alert('data berhasil diubah');</script>";
        		                   	echo "<script>location='ubahprorata.php?id=$idpo';</script>";
        					                     }                  
					...
					?>
		                   ...
		                   <a class="btn btn-primary" href="dataprorata.php?id=<?php echo $idpoproduk ?>">Simpan</a>
