...
if(!isset($_SESSION["admin_mitra"])){
 ...
 $idpoproduk = $_GET['id'];
  ...
  $query = "SELECT COUNT(*) as jumlah,poproduk.idpoproduk,poproduk.namapo,poproduk.status FROM poproduk inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk WHERE pomitra.invoice='$invoice' ";
...
<div class="container row fixed-top navbaru" >
  ...
  <div class="col-2"><a href="datapom.php?invoice=<?php echo $invoice ?>&id=<?= $idpoproduk; ?>"><i class="glyphicon glyphicon-chevron-left"></i></a></div>
...
<?php } ?>
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					$subtotal=0;
					// Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					$sql = mysqli_query($koneksi, "SELECT poproduk.namapo,pokategori.namakategori,pokategori.idpo,podetail.variant,pomitra.idpomitra,pomitra.jumlah,pomitra.invoice,pomitra.total,podetail.harga 
					FROM poproduk inner JOIN pokategori inner join podetail inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail WHERE pomitra.idmitra='$idadmin' and pomitra.invoice='$invoice' order by podetail.idpodetail asc");
					...
					?>
						<tr>
							...
							<td class="align-middle">
								...
								<input type="number" min="0" name="jmlh" class="form-control" style="width: 100px;"> 
 ...
 <?php
			                        ...
			                        $sql = "Select stok from pokategori where idpo='$idpo'";
					                          ...
					                          if($sisa['stok']>=$jmlh){
        					                   ...
        					                   $koneksi->query("UPDATE pokategori set stok=stok-'$jmlh' where idpo='$idpo'");
		                                        ...
		                                        echo "<script>location='ubahmikistock.php?id=$idpoproduk&invoice=$invoice';</script>";
        					                     }
        					                     else{
		                   ...
		                   <a class="btn btn-primary" href="datapom.php?invoice=<?php echo $invoice ?>&id=<?= $idpoproduk; ?>">Simpan</a>
