<?php
    ...
    $ambil = $koneksi->query("SELECT COUNT(*) as jmlh FROM keranjang WHERE idproduk = '$idvariant' AND idmitra = '$idmitra' "); 
    $data  = $ambil->fetch_assoc();

    // if( $data['jmlh'] == '1' && $stock['stock'] > 0 ){
    //     // $koneksi->query("UPDATE keranjang SET jmlh = jmlh + 1, subtotal = subtotal + '$harga' WHERE idproduk = '$idvariant' AND idmitra = '$idmitra'");
    //     $koneksi->query("INSERT INTO keranjang (idkeranjang, idproduk, idmitra, idagen, idreseller, idmarketer, 
    //                                             jmlh, harga, subtotal, tgl, waktu, status, variant)
    //                             VALUES (NULL, '$idvariant', '$idmitra', '', '', '', '1', '$harga', '$harga', 
    //                                     NOW(), '$waktu', 'Active', '$idvariant')");
