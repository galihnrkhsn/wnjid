  ...
  <?php
    if(isset($_POST["save"])){
      ...
      for($x=0; $x<$jumlah_dipilih; $x++){
        //////////CHECK QTY///////////////////
          if($jmlh[$x]>0){
              //////////GET DATA KOKO///////////////////
                ...
                //////////GET DATA SARUNG///////////////////
                $sql_sarung = "SELECT * FROM podetail WHERE idpodetail = '$idpodetail_sarung[$x]'";
                ...
                if($datacocok<>0){
                     $sql_update = $koneksi->query("UPDATE pomitra set jumlah=jumlah + '$jmlh[$x]', total = total+'$total_sarung' where invoice='$invoice' and idpodetail='$idpodetail_sarung[$x]'");
                }else{
                ...
                if($datacocok_koko<>0){
                     $sql_update_koko = $koneksi->query("UPDATE pomitra set jumlah=jumlah + '$jmlh[$x]', total = total+'$total' where invoice='$invoice' and idpodetail='$idpodetail[$x]'");
                }else{
                    $koneksi->query("INSERT INTO pomitra (idpomitra,idmitra,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) VALUES
        ...
        //echo "<script>alert('$idpodetail_sarung[$x], $idpodetail[$x], $jmlh[$x], $datacocok, $datacocok_koko');</script>";
                
                
              
          }// if jumlah tidak kosong
      } // tutup foreach
