...
session_start();
    ...
    $invoice    = $_GET["id"];
    header('Location: '."https://wnj.id/distributor/detailorderb2.php?id=$invoice");
...
<table class="table table-bordered">
  <tr>
   ...
   // $total=0;
    $nomorurut = 1;
    $sql = "SELECT * FROM ordermitra inner join produk on produk.idproduk=ordermitra.idproduk 
...
<tr>
  ...
  <?php endif ?>
    ...
    <?php
        ...
        $sql = "SELECT * FROM ordermitra inner join produk on produk.idproduk = ordermitra.idproduk 
                    WHERE ordermitra.invoice='$invoice' 
                    and ordermitra.jumlah > 0 and produk.idkategori > 0 
  ...
  while ($d25 = $query->fetch_assoc()){
    ...
    <?php
        $ttl52 = 0;
        $sql = "SELECT MAX(produk.harga) as subtotal FROM ordermitra INNER JOIN produk ON produk.idproduk = ordermitra.idproduk WHERE ordermitra.invoice='$invoice' AND ordermitra.jumlah > 0 AND produk.idkategori = 52";
        // $sql = "SELECT * FROM ordermitra INNER JOIN produk ON produk.idproduk = ordermitra.idproduk WHERE ordermitra.invoice AND ordermitra.jumlah > 0 AND produk.idkategori = 52";
        $query = $koneksi->query($sql);
    ...
    <?php
    $totald51=0;
    $sql = "SELECT * FROM ordermitra inner join produk on produk.idproduk=ordermitra.idproduk WHERE ordermitra.invoice='$invoice' and ordermitra.jumlah>0 and produk.idkategori=51";
  $query = $koneksi->query($sql);
  ...
  while ($dfree = $query->fetch_assoc()){
    ...
    $dropship=$pengiriman['berat'];
        ...
        $p = 0;
        $dummy = "SELECT MAX(produk.harga) as harga FROM ordermitra INNER JOIN produk ON produk.idproduk = ordermitra.idproduk WHERE ordermitra.invoice='$invoice' AND ordermitra.jumlah > 0 AND produk.idkategori = 52";
    ...
    if($ongkir==0){
      if($kurir=='Ahsan' or $kurir=='Gosend' or $kurir=='Ambil ke Pusat' or $kurir=='Disatukan') {
            $ceksql = "SELECT * FROM ordermitra INNER JOIN produk ON produk.idproduk = ordermitra.idproduk WHERE ordermitra.invoice='$invoice' AND ordermitra.jumlah > 0 AND produk.idkategori = 52";
