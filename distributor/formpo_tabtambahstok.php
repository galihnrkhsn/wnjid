...
if($sisa['stok']>$jmlh[$x]){                                             	
		...
		if($datacocok>=1){
		      $sql = $koneksi->query("UPDATE pomitra set jumlah=jumlah + '$jmlh[$x]' WHERE idpodetail='$idpodetail[$x]' and invoice ='$invoice'");
			...
			(null,'$idadmin','$idpoproduk','$idpo','$idpodetail[$x]','$jmlh[$x]','$total','D$idpoproduk-$idadmin','Belum DP',NOW(),'$waktu')"); 
			$koneksi->query("UPDATE pokategori set stok=stok-'$jmlh[$x]' where idpo='$idpo'");
		}                                             	
}        					                   
        					               }
