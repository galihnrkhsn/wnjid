...
include "koneksi.php";
	...
	if(isset($_POST['kirim'])){
	 ...
	 $sql = mysqli_query($koneksi, "SELECT iddropship FROM podropship_max order by iddropship desc limit 1");
            ...
            (null,'$idadmin','0','0','0','$idpoproduk','MXD$idadmin-$nobaru','Pending','$namapengirim','$tlppengirim','$namapenerima','$tlppenerima','$alamatpenerima','$keterangan','$ekspedisi')";
          $sql = mysqli_query( $koneksi, $query);  
		
		            if ( $sql == TRUE ){
					echo "<script>alert('Berhasil Disimpan, Silahkan input PO yang diinginkan');</script>";
						echo "<script>location='formpomaxx.php?id=51&inv=MXD$idadmin-$nobaru'</script>";
				}else{
