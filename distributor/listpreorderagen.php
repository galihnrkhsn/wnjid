	...
	-->		<div class="table-responsive">
				<table class="table table-bordered">
					...
					if(isset($_GET["cari"])){
					    ...
					    $idagen=$_SESSION['admin_mitra']['idadmin'];
					    // Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					    $sql = mysqli_query($koneksi, "SELECT poagen.tgl,poagen.status,poagen.invoice,poproduk.namapo,poproduk.idpoproduk FROM `poagen` inner join poproduk
                        on poagen.idpoproduk=poproduk.idpoproduk where poagen.idagen='$idagen' and poproduk.status='open' where poproduk.namapo LIKE '%$namapo%' order by poagen.tgl desc limit 1");
				    	...
				    	// Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
				    	$sql = mysqli_query($koneksi, "SELECT poagen.tgl,poagen.status,poagen.invoice,poproduk.namapo,poproduk.idpoproduk FROM `poagen` inner join poproduk
                        on poagen.idpoproduk=poproduk.idpoproduk where poagen.idagen='$idagen' and poproduk.status='open' order by poagen.tgl desc");
					    ...
					    $limit_start = ($page - 1) * $limit;
					    // Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					    $sql = mysqli_query($koneksi, "SELECT DISTINCT poagen.tgl,poproduk.namapo,poagen.status,poagen.invoice,poagen.idagen,poagen.idpoproduk,mitraagen.namaagen,mitrareseller.namaagen as namareseller,mitramarketer.namaagen as namamarketer FROM poagen LEFT JOIN poproduk on poagen.idpoproduk=poproduk.idpoproduk LEFT JOIN mitraagen on poagen.idagen=mitraagen.idmitraagen LEFT JOIN mitrareseller on poagen.idreseller=mitrareseller.idmitrareseller LEFT JOIN mitramarketer on poagen.idmarketer=mitramarketer.idmitramarketer LEFT JOIN admin_mitra on mitraagen.idadmin=admin_mitra.idadmin and mitrareseller.idadmin=admin_mitra.idadmin and mitramarketer.idadmin=admin_mitra.idadmin where mitraagen.idadmin='$idadmin' or mitrareseller.idadmin='$idadmin' or mitramarketer.idadmin='$idadmin' order by poproduk.idpoproduk desc limit 20");
					    $no = $limit_start + 1; // Untuk penomoran tabel
					}
					...
					?>
						<tr>
							...
							<td class="align-middle"><?php echo $data['namaagen']; ?><?php echo $data['namareseller']; ?><?php echo $data['namamarketer']; ?></td>
							<td class="align-middle"><?php echo $data['status']; ?></td>
							<td class="align-middle"><a href="datapoagen.php?idagen=<?php echo $data['idagen']; ?>&id=<?php echo $data['idpoproduk']; ?>"><?php echo $data['invoice']; ?></a></td>
							...
							?><a href="listdropship.php?invoice=<?php echo $data['invoice']; ?>"><?php echo $dropship['dropship']; ?></a>
							</td>-->
				...
				if(isset($_POST["approve"])){
				    ...
				    $koneksi->query("UPDATE poagen set status='Belum DP' where invoice='$invoice' ");
    	            echo "<script>alert('PO Agen telah di Approve');</script>";
		            echo "<script>location='listpreorderagen.php';</script>";
				}
