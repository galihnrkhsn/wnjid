		...
		<div class="container">
			<div class="panel-body">
				<div class="row">
					<div class="col-md-6">
						<p align="right">
							<input type="radio" onclick="javascript:window.location.href='formpengirimanb.php?id=<?php echo $invoice; ?>&berat=<?php echo $berat; ?>&jenis=<?= $jenis ?>'; "> Dropship
							<input type="radio" onclick="javascript:window.location.href='formpengiriman2.php?id=<?php echo $invoice; ?>&berat=<?php echo $berat; ?>&jenis=<?= $jenis ?>';"  checked="checked"> Kirim Ke Alamat Pribadi
						</p>
						<form method="post">
							<div class="form-group">
								<label>Nama Pengirim</label>
								<input type="text" class="form-control" name="namapengirim" value="<?= $dataUser['namamitra']; ?>" required>
							</div>
							<div class="form-group">
								<label>Telepon Pengirim</label>
								<input type="text" class="form-control" name="tlppengirim" value="<?= $dataUser['whatsapp']; ?>" required maxlength="17">
							</div>
							<hr>
							<div class="form-group">
								<label>Nama Penerima</label>
								<input type="text" class="form-control" name="namapenerima" value="<?= $dataUser['namamitra']; ?>" required>
							</div>
							<div class="form-group">
								<label>Telepon Penerima</label>
								<input type="text" class="form-control" name="tlppenerima" value="<?= $dataUser['whatsapp']; ?>" required maxlength="17">
							</div>
							<div class="form-group">
								<label>Alamat</label>
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" name="kodepos" value="<?= $dataUser['kodepos']; ?>" required>
							</div>
							<div class="form-group">
	...
	if(isset($_POST['kirim'])){
		...
		if ($pengirimancek) {
			...
			$query 		 = "INSERT INTO orderpengiriman (idorderp, namapengirim, tlppengirim, namapenerima, tlppenerima, alamat, provinsi, kota, kecamatan,
							...
							VALUES (NULL, '$namapengirim', '$tlppengirim', '$namapenerima', '$tlppenerima', '$alamat', '$provinsi', '$kabupaten',
											'$kecamatan', '$ekspedisi', '$layananku', '$berat', '$ongkir', 'tidak', '$kodepos', '$invoice', 
											'$total', 0, NOW())"; 
		...
		if ( $sql == TRUE ){
			if (substr($invoice,0,1)=="F") {
				...
				echo "<script>alert('Data berhasil ditambah');</script>";
				echo "<script>location='detailorder3.php?id=$invoice'</script>";
			} elseif ($jenis == 'Bundling 5') { 
				echo "<script>alert('Data berhasil ditambah');</script>";
				echo "<script>location='detailorder5.php?id=$invoice'</script>";
			} elseif ($jenis == 'Flash') { 
				echo "<script>alert('Data berhasil ditambah');</script>";
				echo "<script>location='detailorderb2.php?id=$invoice&jenis=$jenis'</script>";
			} else{
				echo "<script>alert('data berhasil ditambah');</script>";
				echo "<script>location='detailorderb2.php?id=$invoice'</script>";
			}
		}else{
