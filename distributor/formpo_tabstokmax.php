...
<?php
	...
	for($y=0;$y<$jumlah_dipilih;$y++){		
		...
		$hasil2 += $jmlh[$y];
			...
			if ($hasil>3) {
				echo "<script>alert('Jumlah Dress lebih dari 3pcs');</script>";
				echo "<script>location='formpo_tabstokmax.php?id=$idpoproduk';</script>";
				return false;
			}
			if ($hasil2>3) {
				echo "<script>alert('Jumlah Khimar lebih dari 3pcs');</script>";
				echo "<script>location='formpo_tabstokmax.php?id=$idpoproduk';</script>";
				return false;
			}		
				...
				$sisa = $query_sisa->fetch_assoc();
					...
					if($sisa['stok']>$jmlh[$x]){
						$koneksi->query("INSERT into pomitra (idpomitra,idmitra,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) 
							values (null,'$idadmin','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total','D$idpoproduk-$idadmin','Belum DP',NOW(),'$today')");
