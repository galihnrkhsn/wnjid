...
<body>
        ...
        <div class="col-sm-12 mb-3">
          <div class="row">
            ...
            <div class="col-sm-4">
              ...
              ?>
                <input type="hidden" name="idpodetail_sarung[]" id="" value="<?= $row['idpodetail'] ?>">
                <input type="text" value="<?= $row['variant'] ?>" class="form-control" id="<?= $row['variant'] ?>" disabled>
              <?php } ?>
            </div>

            <div class="col-sm-6">
              <select name="idpodetail[]" id="" class="form-select form-control-sm">
                <?php
                  $sql = "SELECT * FROM poproduk
                          INNER JOIN pokategori ON poproduk.idpoproduk = pokategori.idpoproduk
                          INNER JOIN podetail ON pokategori.idpo = podetail.idpo
        ...
        <div class="col-sm-12 mb-3">
          <div class="row">
            <div class="col-sm-12 col-sm-12 d-flex align-items-center gap-2">
              ...
              <a class="btn btn-sm btn-success mb-1" style="padding: 0 .2rem;" id="buttonAdd1"><i class="bi bi-plus" style="font-size: 1rem"></i></a>
            </div>
        ...
        <div class="col-sm-12 mb-3">
          <div class="row">
            <div class="col-sm-12 col-sm-12 d-flex align-items-center gap-2">
              ...
              <a class="btn btn-sm btn-success mb-1" style="padding: 0 .2rem;" id="buttonAdd2"><i class="bi bi-plus" style="font-size: 1rem"></i></a>
            </div>
        ...
        <div class="col-sm-12 mb-3">
          <div class="row">
            <div class="col-sm-12 col-sm-12 d-flex align-items-center gap-2">
              ...
              <a class="btn btn-sm btn-success mb-1" style="padding: 0 .2rem;" id="buttonAdd3"><i class="bi bi-plus" style="font-size: 1rem"></i></a>
            </div>
    ...
    if(isset($_POST["save"])){
      ...
      for($x = 0; $x < $jumlah_dipilih; $x++) {
        if($jmlh[$x] > 0) {
          ...
          $sql_sarung = "SELECT * FROM podetail WHERE idpodetail = '$idpodetail[$x]'";
          $query_sarung = $koneksi->query($sql_sarung);
          $detail_sarung = $query_sarung->fetch_assoc();
          $harga_sarung = $detail_sarung["harga"];
          $idpo_sarung = $detail_sarung["idpo"];
          
          $total = $jmlh[$x]*$harga;
          $total_sarung = $jmlh[$x]*$harga_sarung;
          
          // QUERY KOKO
          $ambil_koko = $koneksi->query("SELECT * FROM pomitra WHERE invoice = '$invoice' AND idpodetail = '$idpodetail[$x]'");
          $datacocok_koko = $ambil_koko->num_rows;
          if($datacocok_koko<>0) {
            $sql_update_koko = $koneksi->query("UPDATE pomitra SET jumlah = jumlah + '$jmlh[$x]' WHERE invoice = '$invoice' AND idpodetail = '$idpodetail[$x]'");
          } else{
            $koneksi->query("INSERT INTO pomitra (idpomitra,idmitra,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu)
                              VALUES (NULL,'$idadmin','$idpoproduk','$idpo','$idpodetail[$x]','$jmlh[$x]','$total','$invoice','Belum DP',NOW(),'$waktu')
                            ");
          }

          // QUERY SARUNG
          $ambil=$koneksi->query("SELECT * FROM pomitra WHERE invoice = '$invoice' AND idpodetail = '$idpodetail_sarung[$x]'");
          $datacocok=$ambil->num_rows;
          if($datacocok<>0) {
            $sql_update = $koneksi->query("UPDATE pomitra SET jumlah = jumlah + '$jmlh[$x]' WHERE invoice = '$invoice' AND idpodetail = '$idpodetail_sarung[$x]'");
          } else {
            $koneksi->query("INSERT INTO pomitra (idpomitra,idmitra,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) VALUES
                            (NULL,'$idadmin','$idpoproduk','$idpo_sarung','$idpodetail_sarung[$x]','$jmlh[$x]','$total_sarung','$invoice','Belum DP',NOW(),'$waktu')
                          ");
          }
        }
      }
  ...
  <script>
        ...
        <div class="row">
          <div class="col-sm-4">
            <?php
              ...
              while($row = $query->fetch_assoc()) {
            ?>
              <input type="hidden" name="idpodetail_sarung[]" value="<?php echo $row['idpodetail']; ?>">
              <input type="text" value="<?= $row['variant'] ?>" class="form-control" id="<?= $row['variant'] ?>" disabled>
            <?php } ?>
          </div>
    ...
    // Sarung Etnic Rpyal Black
    const buttonAdd1 = document.getElementById("buttonAdd1");
    const newForm1 = document.getElementById("newForm1");
    let formCount1 = 0;
    ...
    buttonAdd1.addEventListener('click', function() {
      ...
      newForm1.appendChild(form);
    });

    // Sarung Etnic Soft Lavender
    const buttonAdd2 = document.getElementById("buttonAdd2");
    const newForm2 = document.getElementById("newForm2");
    let formCount2 = 0;
    ...
    buttonAdd2.addEventListener('click', function() {
      ...
      newForm2.appendChild(form);
    });

    // Sarung Etnic Soft Tosca
    const buttonAdd3 = document.getElementById("buttonAdd3");
    const newForm3 = document.getElementById("newForm3");
    let formCount3 = 0;
