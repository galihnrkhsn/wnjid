  ...
  <?php $stok= $stok['stok']; } ?> -->
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					FROM poproduk inner JOIN pokategori inner join podetail inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail WHERE pomitra.idmitra='$idadmin' and pomitra.idpoproduk='$idpoproduk' and pomitra.invoice='$invoice' ORDER BY pomitra.idpomitra ASC");
					...
					?>
						<tr>
							...
							<td>
                    <input type="checkbox" class="check-item" name="idpomitra_check[]" value="<?php echo $data['idpomitra']; ?>" class="form-control">
							</td> 
							...
							<td class="align-middle">
							<input type="text" name="custom[]" value="<?php echo $data['custom']; ?>" maxlength="10">
				...
				<br>
		                   ...
		                   <button type="submit" class="btn btn-danger" name="hapuspocheck"  onclick="return confirm('Yakin Akan Menghapus Pesanan? Barang Akan Kembali Ke Stok');"><span class="fas fa-trash"></span>Hapus Check</button>  
		        </form>        		            
  ...
  $namanya = addslashes($custom[$x]);
            if(!preg_match("/^\S{1,}$/", $namanya)){
              ...
              return false;
                       ...
                       for($x=0;$x<$jmlhcustom;$x++){
                       	$namanya = addslashes($custom[$x]);
		                   $koneksi->query("UPDATE pomitra set custom='$namanya'  WHERE idpomitra='$idpomitra[$x]';");
...
if(isset($_POST["hapuspocheck"])){
				      include "koneksi.php";
				      $idpomitra_check = $_POST['idpomitra_check'];
				      $jumlah_dipilih=count($idpomitra_check);
				      for($x=0;$x<$jumlah_dipilih;$x++){

  $query = "SELECT * from pomitra Where idpomitra='$idpomitra_check[$x]'";
  $sql = mysqli_query($koneksi, $query);  
  $data = mysqli_fetch_array($sql);
  ...
  $idpo=$data['idpo'];
  $invoice=$data['invoice'];
$sqlnya = $koneksi->query("DELETE from pomitra where idpomitra='$idpomitra_check[$x]';");
$sqlstok = $koneksi->query("UPDATE pokategori set stok=stok+1 where idpo='$idpo'");
}          
	echo "<script>alert('Data Berhasil dihapus');</script>";
		echo "<script>location='ubahbrooch_baru.php?id=$idpoproduk&invoice=$invoice';</script>";				        
...
<script type="text/javascript">
	     function checkAllDB(box) 
  {
   ...
   if (box.checked) { // jika checkbox teratar dipilih maka semua tag input juga dipilih
    var idorder_dbcheck = document.getElementsByName("idpomitra_check[]");
