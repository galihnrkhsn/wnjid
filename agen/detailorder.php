...
<div class="container mt-2">
    ...
    <h4 class="text-center"><?= $dataAgen['namaagen'] ?> ( Cust ID : <?= $dataAgen['idmitraagen']; ?> )</h4><br>
    ...
    $sqlcek2 = "SELECT * FROM orderagen WHERE invoice='$invoice' ";
...
<table class="table table-bordered">
  <tr>
   ...
   // $total=0;
    $nomorurut = 1;
    $sql = "SELECT * FROM orderagen inner join produk on produk.idproduk=orderagen.idproduk 
...
<tr>
  ...
  <?php endif ?>
    ...
    <?php
        ...
        $sql = "SELECT * FROM orderagen inner join produk on produk.idproduk = orderagen.idproduk 
                    WHERE orderagen.invoice='$invoice' 
                    and orderagen.jumlah > 0 and produk.idkategori > 0 
  ...
  while ($d15 = $query->fetch_assoc()){
    ...
    <?php
    $totald17=0;
    $sql = "SELECT * FROM orderagen inner join produk on produk.idproduk=orderagen.idproduk WHERE orderagen.invoice='$invoice' and orderagen.jumlah>0 and produk.idkategori=17";
  $query = $koneksi->query($sql);
  ...
  while ($d25 = $query->fetch_assoc()){
    ...
    <?php
        $ttl52 = 0;
        $sql = "SELECT MAX(produk.harga) as subtotal FROM orderagen INNER JOIN produk ON produk.idproduk = orderagen.idproduk WHERE orderagen.invoice='$invoice' AND orderagen.jumlah > 0 AND produk.idkategori = 52";
        // $sql = "SELECT * FROM orderagen INNER JOIN produk ON produk.idproduk = orderagen.idproduk WHERE orderagen.invoice AND orderagen.jumlah > 0 AND produk.idkategori = 52";
        $query = $koneksi->query($sql);
    ...
    <?php
    $totald51=0;
    $sql = "SELECT * FROM orderagen inner join produk on produk.idproduk=orderagen.idproduk WHERE orderagen.invoice='$invoice' and orderagen.jumlah>0 and produk.idkategori=51";
  $query = $koneksi->query($sql);
  ...
  while ($dfree = $query->fetch_assoc()){
    ...
    $dropship=$pengiriman['berat'];
        ...
        $p = 0;
        $dummy = "SELECT MAX(produk.harga) as harga FROM orderagen INNER JOIN produk ON produk.idproduk = orderagen.idproduk WHERE orderagen.invoice='$invoice' AND orderagen.jumlah > 0 AND produk.idkategori = 52";
    ...
    if($ongkir==0){
      if($kurir=='Ahsan' or $kurir=='Gosend' or $kurir=='Ambil ke Pusat' or $kurir=='Disatukan') {
            $ceksql = "SELECT * FROM orderagen INNER JOIN produk ON produk.idproduk = orderagen.idproduk WHERE orderagen.invoice='$invoice' AND orderagen.jumlah > 0 AND produk.idkategori = 52";
        ...
        echo "Diskon DB 25% : Rp. -$tdiskona<br>";
      ...
      //echo "<p align='right'>Diskon Manual : Rp. -$diskonramadhan<br>";
            // $sql = "SELECT * FROM orderagen inner join produk on produk.idproduk=orderagen.idproduk WHERE namaproduk LIKE '%Esacus Dress%' AND orderagen.invoice='$invoice' and orderagen.jumlah>0 ";
            // $query = $koneksi->query($sql);
    ...
    //     }
    //     echo "Estimasi Ongkir : Rp. $tongkir<br>";
    //     echo "Diskon DB 25% : Rp. -$tdiskon52<br>";
...
?>
      <center><button type="submit" class="btn btn-sm" name="cari" id="linkmiki<?= $tampilkan['idorder']; ?>">
        <?php
            ...
            //         WHERE orderagen.invoice = '$invoice' AND orderagen.jumlah > 0";
            // $query = $koneksi->query($sql);
            // $grandTotalToShow = $tgrandtotal; // Default grand total value
