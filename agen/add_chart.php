...
$ambil=$koneksi->query("SELECT COUNT(*) as jmlh FROM keranjang where idproduk='$idproduk' and idagen='$idagen' "); 
$data=$ambil->fetch_assoc();

        if($data['jmlh']=='1' and $stock['stock']>0){
            $koneksi->query("UPDATE keranjang SET jmlh=jmlh+1,subtotal=subtotal+'$harga' where idproduk='$idproduk' and idagen='$idagen' ");
      ...
      VALUES (null,'$idproduk','','$idagen','','','1','$harga','$harga',NOW(),'$waktu','Active')");
      $koneksi->query("UPDATE produk SET stock=stock-1 where idproduk='$idproduk'");
