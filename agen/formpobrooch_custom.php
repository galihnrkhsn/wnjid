...
$query_variant = "SELECT podetail.harga, podetail.idpo
  ...
  if (preg_match("/^\S{1,}$/", $namanya)) {
    $sqlinsert = $koneksi->query("insert into pomitra (idpomitra,idmitraagen,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl,waktu) values
    (null,'$idmitraagen','$idpoproduk','$idpo','$idpodetail[$x]','$jumlah[$x]','$namanya','$total',
    'BA$idpoproduk-$idmitraagen-$nobaru','Belum Acc DB',NOW(),'$waktu')");
...
if ($sqlinsert) {
  echo "<script>location='datapom.php?id=$idpoproduk&invoice=BA$idpoproduk-$idmitraagen-$nobaru';</script>";  
}
