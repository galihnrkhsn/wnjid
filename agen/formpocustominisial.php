...
$rowCek = $koneksi->query("
    ...
    FROM pomitra
    WHERE idpoproduk = '$idpoproduk' AND idmitraagen = '$idmitraagen'
")->fetch_assoc();
...
if (isset($_POST['save'])) {
    try {
        $checkExists = $koneksi->query("
            ...
            WHERE idmitraagen='$idmitraagen' AND idpoproduk='$idpoproduk'
            AND idmitrareseller IS NULL AND idmitramarketer IS NULL
        ")->fetch_assoc()['count'];
        ...
        if (isset($_GET['invoice'])) {
            ...
            $invoice = 'ACI' . $idpoproduk . '-' . $idmitraagen . generateAngkaAcak(5);
        }
        ...
        foreach ($idpodetail as $x => $idDetailNow) {
            ...
            foreach ($customArr as $idx => $inisial) {
                ...
                $ok = $koneksi->query("
                    INSERT INTO pomitra
                        (idpomitra, idmitraagen, idpoproduk, idpo, idpodetail, jumlah, total, custom, font, invoice, status, tgl, waktu)
                    VALUES
                        (NULL, '$idmitraagen', '$idpoproduk', '$idpo', '$idDetailNow',
