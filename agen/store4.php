    ...
    <div class="container">
        ...
        <?php
            if(isset($_GET["cari"])){
                ...
                $sql            = mysqli_query($koneksi, "SELECT products.namaproduk, variants.* FROM variants 
                                                            ...
                                                            AND status <> 1
                                                            AND products.access != 1 
                                                            ORDER BY variants.updated_at DESC, idproduk DESC 
                ...
                $sql            = mysqli_query($koneksi, "SELECT products.namaproduk, variants.* FROM variants 
                                                            INNER JOIN products ON variants.idproducts = products.id
                                                            WHERE variants.status = '$status' 
                                                            AND variants.stock > 0
                                                            AND products.access != 1
                                                            ORDER BY variants.updated_at DESC, products.id DESC 
                                                            LIMIT ".$limit_start.",".$limit
                                                        );
                $no             = $limit_start + 1;
            }
        ...
        ?>
            <div class="col-lg-3 col-md-6 col-sm-6 col-xs-6 col-6" style="margin-bottom: 2%;">
                <div class="card">
                    <?php if ($data['foto'] <> ""): ?>
                        <img class="card-img-top fixed-size-img" src="../distributor/foto/produk/<?= $data['foto']; ?>" alt="Card image">
                    <?php else: ?>
