							...
							<tbody>
								<?php 
									...
									$sql 		= mysqli_query($koneksi, "SELECT 
																			...
																			where orderagen.idmitraagen = '$idmitra' 
																			and (orderagen.payment<>'Lunas' or orderagen.payment<>'LUNAS') 
																			GROUP BY invoice 
								...
								<tr>
									...
									<td>
										<b><?php echo $data['tgl']; ?></b>
										<br>
										DB: <?php echo $data['namamitra'];?>
										<br> 
										<b>Payment : <?php echo $data['payment']; ?></b>
									</td>
									<td>
										Inv :
											...
											<?php elseif ($data['jenis'] == 'Flash'): ?>
												<a class="text-primary" href="detailorder2.php?id=<?= $data['invoice']; ?>&jenis=Flash">#<?= $data['invoice'] ?></a>
											<?php elseif (strtotime($data['tgl']) > strtotime('2024-10-02 23:59:59') && substr($data['invoice'], 0, 2) == "AP"): ?>
											...
											<?php endif ?>
											<br>
											Status : <?php echo $data['statusnya']; ?>
									</td>
								</tr>
								<?php } ?>
							</tbody>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="tb_datareseller">
							...
							<?php
								...
								$sql = mysqli_query($koneksi, "SELECT DISTINCT mitraagen.namaagen, orderagen.invoice, orderagen.payment, 
																orderagen.status,orderagen.tgl, admin_mitra.namamitra
														FROM `orderagen` 
														INNER JOIN mitraagen 
														ON mitraagen.idmitraagen = orderagen.idmitraagen 
                                                        INNER JOIN admin_mitra
                                                        ON admin_mitra.idadmin = mitraagen.idadmin
														WHERE orderagen.idmitraagen='$idmitra' 
														AND (orderagen.payment = 'Lunas' OR orderagen.payment='LUNAS') 
							...
							?>
								<tr>
									<td style="width: 5%"><?= $no++; ?></td>											
									<td>
										<b><?php echo $data2['tgl']; ?></b>
										<br>
										DB: <?php echo $data2['namamitra'];?>
										<br> 
										<b>Payment : <?php echo $data2['payment']; ?></b>
									</td>
									<td>Inv: 
										<?php if (substr($data2['invoice'],0,1) == "F"): ?>
											<a class="text-primary" href="detailorder_get.php?id=<?php echo $data2['invoice']; ?>">#<?php echo $data2['invoice']; ?></a>
										<?php elseif (substr($data2['invoice'], 0, 2) === "AP") : ?>
											<a class="text-primary" href="dataorder.php?id=<?php echo $data2['invoice']; ?>">#<?php echo $data2['invoice']; ?></a>
										<?php else : ?>
											<a class="text-primary" href="detailorder.php?id=<?php echo $data2['invoice']; ?>">#<?php echo $data2['invoice'];?></a>
										<?php endif ?>
										<br>Status : <?php echo $data2['status']; ?><br>
										<? if ($data2['status'] == 'Sudah Diterima!') : ?>
											<!-- <p class="text-success">Barang Sudah diterima!</p> -->
										<? else : ?>
											<form method="post">
												<input type="hidden" name="id" value="<?php echo $data2['invoice']; ?>">
	...
	<?php
		if(isset($_POST["done"])){
			try {
				...
				$updateQuery = "UPDATE orderagen SET status = 'Sudah Diterima!', payment = 'Lunas' WHERE invoice = '$invoiceNilai'";
