...
if(!isset($_SESSION["mitraagen"])){
 ...
 $idpoproduk = $_GET['id'];
  ...
  $query = "SELECT COUNT(*) as jumlah,poproduk.idpoproduk,poproduk.namapo,poproduk.status FROM poproduk inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk WHERE poproduk.idpoproduk='$idpoproduk' AND pomitra.idmitra='$idmitraagen'";
...
<p align="left">Nama Agen  : <?php echo $_SESSION["mitraagen"]["namaagen"]; ?> </p>
...
$sql2 = mysqli_query($koneksi, "SELECT poproduk.namapo,pokategori.namakategori,podetail.variant,pomitra.idpomitra,pomitra.jumlah,pomitra.invoice,pomitra.total,podetail.harga 
					FROM poproduk inner JOIN pokategori inner join podetail inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail WHERE pomitra.idmitraagen='$idmitraagen' and pomitra.idpoproduk='$idpoproduk'");
...
$sql = mysqli_query($koneksi, "SELECT poproduk.namapo,pokategori.namakategori,podetail.variant,pomitra.idpomitra,pomitra.jumlah,pomitra.invoice,pomitra.total,podetail.harga 
		FROM poproduk inner JOIN pokategori inner join podetail inner join pomitra on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail WHERE pomitra.idmitraagen='$idmitraagen' and pomitra.idpoproduk='$idpoproduk' order by podetail.idpodetail asc");
...
?>
	<tr>
        ...
        if(isset($_POST["edit"])){
            ...
            echo "<script>location='adminrevisi.php?id=$idpoproduk';</script>";
                					                     }
        elseif(isset($_POST["hapus"])){
        	$idpomitra= $_POST['idpomitra'];
        	$koneksi->query("delete from pomitra where idpomitra='$idpomitra';");
            echo "<script>alert('data berhasil diubah');</script>";
            echo "<script>location='adminrevisi.php?id=$idpo';</script>";
                					                     }                  
