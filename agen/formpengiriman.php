<?php 
    ...
    $getAgen     = $queryAgen->fetch_assoc();
?>
  
  <html lang="en">
<head>
    <title>Mitra <?= $getAgen['namaagen']?>| Wanoja</title>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="container">
        <div class="panel-body">
            <div class="row">
                <div class="col-md-6">
                    <p align="right"><input type="radio" onclick="javascript:window.location.href='formpengirimanb.phpid=<?php echo $invoice; ?>&berat=<?php echo $berat; ?>'; " checked="checked"> Dropship  <input type="radio" onclick="javascript:window.location.href='formpengiriman2.php?id=<?php echo $invoice; ?>&berat=<?php echo $berat; ?>'; "> Kirim Ke Alamat Pribadi</p>
                    <form method="post">
                        <div class="form-group">
                            <label>Nama Pengirim</label>
                            <input type="text" class="form-control" name="namapengirim" value="<?= $getAgen['namaagen'] ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Telepon Pengirim</label>
                            <input type="text" class="form-control" name="tlppengirim" value="<?= $getAgen['whatsapp'] ?>" required maxlength="17">
                        </div>
                        <hr>
                        <div class="form-group">
...
<?php
	...
	if(isset($_POST['kirim'])){
        ...
        $sql 		= "SELECT SUM(orderagen.subtotal) AS subtotal, SUM(orderagen.jumlah) AS jumlah, variants.jenis 
						FROM orderagen 
        ...
        if ($pengirimancek) {
            ...
            $query       = "INSERT INTO orderpengiriman (idorderp, namapengirim, tlppengirim, namapenerima, tlppenerima, alamat, provinsi, kota, kecamatan, 
                                ...
                                values (null,'$namapengirim', '$tlppengirim', '$namapenerima', '$tlppenerima', '$alamat', '$provinsi', '$kabupaten', '$kecamatan', 
                                '$ekspedisi', '$layananku', '$berat', '$ongkir', 'ya', '$kodepos', '$invoice', '$total', 0, NOW())"; 
            ...
            (null,'$namapengirim','$tlppengirim','$namapenerima','$tlppenerima','$alamat','$provinsi','$kabupaten','$kecamatan','$ekspedisi','$layananku','$berat','$ongkir','ya','$kodepos','$invoice','$total',0,NOW())";
            $sql = mysqli_query( $koneksi, $query); 
        }
