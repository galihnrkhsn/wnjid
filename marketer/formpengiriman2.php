<?php 
	...
	$queryMarketer      = $koneksi->query("SELECT * FROM mitramarketer WHERE idmitramarketer = '$idmitramarketer'");
	$dataMarketer       = $queryMarketer->fetch_assoc();

	$provinsi			= $dataMarketer["provinsi"];
	$kabupaten			= $dataMarketer["kota"];
	$kecamatan			= $dataMarketer["kecamatan"];
?>
  
<html lang="en">
<head>
	<title>Mitra <?= $dataMarketer['namaagen']; ?>| WNJ.ID</title>
	<meta charset="utf-8">
	...
	<?php include "assets/components/Navbar/navbar.php"; ?>
		...
		<div class="container">
			<div class="panel-body">
				<div class="row">
					<div class="col-md-6">
						<p align="right"><input type="radio" onclick="javascript:window.location.href='formpengiriman.php?id=<?php echo $invoice; ?>&berat=<?php echo $berat; ?>'; "> Dropship  <input type="radio" onclick="javascript:window.location.href='formpengiriman2.php?id=<?php echo $invoice; ?>&berat=<?php echo $berat; ?>';"  checked="checked"> Kirim Ke Alamat Pribadi</p>
						<form method="post">
							<div class="form-group">
								<label>Nama Pengirim</label>
								<input type="text" class="form-control" name="namapengirim" value="<?= $dataMarketer['namaagen']; ?>" required>
							</div>
							<div class="form-group">
								<label>Telepon Pengirim</label>
								<input type="text" class="form-control" name="tlppengirim" value="<?= $dataMarketer['whatsapp']; ?>" required maxlength="17">
							</div>
							<hr>
							<div class="form-group">
								<label>Nama Penerima</label>
								<input type="text" class="form-control" name="namapenerima" value="<?= $dataMarketer['namaagen']; ?>" required>
							</div>
							<div class="form-group">
								<label>Telepon Penerima</label>
								<input type="text" class="form-control" name="tlppenerima" value="<?= $dataMarketer['whatsapp']; ?>" required maxlength="17">
							</div>
							<div class="form-group">
								<label>Alamat</label>
								<textarea class="form-control" name="alamat" required><?= $dataMarketer['alamat']; ?></textarea>
							</div>
							<div class="form-group">
								<label>Kode POS</label>
								<input type="text" class="form-control" name="kodepos" value="<?= $dataMarketer['kodepos']; ?>" required>
							</div>
							<div class="form-group">
								<label for="prov">Provinsi Tujuan</label><br>
								<select class="form-control" id="prov" name="prov" required>
									<?php
										$idprov	= $dataMarketer["provinsi"];
										if($idprov==''){
...
include "koneksi.php";
	...
	if(isset($_POST['kirim'])){        
		...
		$sql 		= "SELECT SUM(ordermarketer.subtotal) AS subtotal, SUM(ordermarketer.jumlah) AS jumlah, variants.jenis 
						FROM ordermarketer 
		...
		if ($pengirimancek) {
			...
			$query 			= "INSERT INTO orderpengiriman 
									(idorderp,namapengirim,tlppengirim,namapenerima,tlppenerima,alamat,provinsi,kota,kecamatan,
										ekspedisi,layanan,berat,ongkir,dropship,kodepos,invoice,total,diskonramadhan,tgl) 
								VALUES
									(null,'$namapengirim','$tlppengirim','$namapenerima','$tlppenerima','$alamat','$provinsi',
									'$kabupaten','$kecamatan','$ekspedisi','$layananku','$berat','$ongkir','ya','$kodepos',
									'$invoice','$total', 0, NOW())"; 
		$sql = mysqli_query( $koneksi, $query); 
		}
		else{
			$query = "INSERT INTO orderpengiriman 
							(idorderp,namapengirim,tlppengirim,namapenerima,tlppenerima,alamat,provinsi,kota,kecamatan,ekspedisi,
							layanan,berat,ongkir,dropship,kodepos,invoice,total,diskonramadhan,tgl) 
						VALUES
							(null,'$namapengirim','$tlppengirim','$namapenerima','$tlppenerima','$alamat','$provinsi','$kabupaten',
							'$kecamatan','$ekspedisi','$layananku','$berat','$ongkir','ya','$kodepos','$invoice','$total',0,NOW())";
			$sql = mysqli_query( $koneksi, $query); 
		}  
		if ( $sql == TRUE ){
			if (substr($invoice, 0, 2) === "MP") {
