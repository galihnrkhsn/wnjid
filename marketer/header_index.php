...
$idmitramarketer=$_SESSION["mitraagen"]["idmitramarketer"];
  $keranjang=0;
$sikat=$koneksi->query("SELECT * FROM keranjang where idmarketer='$idmitramarketer' and jmlh>0 "); 
while($count=$sikat->fetch_assoc()){
     $keranjang+=$count['jmlh'];
}

$ambiljmlh=$koneksi->query("SELECT COUNT(*) as jmlh FROM tinbox where idmitramarketer='$idmitramarketer' and status='Belum Dibaca'"); 
...
<body>
   ...
   <?php 
     $mitra= $_SESSION['mitraagen']['idmitramarketer'];

     $ambil=$koneksi->query("SELECT namaagen FROM mitramarketer  where idmitramarketer= '$mitra'"); 
...
<div class="jumbotron custommargin">
  <div class="row ">
    ...
    <div class="col-7">
      <b>Marketer <?php echo $data['namaagen'] ?></b><br>Mitra Marketer
    </div>
...
<hr style="display: flex; margin: 0 0 2.5rem">
