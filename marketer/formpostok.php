                                ...
                                while ($row = $query->fetch_assoc()) {
                                    ...
                                    <div class="form-group">
                                        <label>
                                            ...
                                            <?php if ($idpoproduk <> 362 and $idpoproduk <> 382 and $idpoproduk <> 394 and $idpoproduk <> 395 and $idpoproduk <> 399 && !in_array($row['idpo'], $excluded_sarung)) :?>
                                                (<?= $row['stok'] ?>)
                                            <?php else :?>
                                            <?php endif; ?>
                                        </label>
        ...
        if (isset($_POST["save"])) {
            ...
            try {
                ...
                for($x = 0; $x < $jumlah_dipilih; $x++){
                    ...
                    if ($sisa['stok'] >= $jmlh[$x]) {
                        $koneksi->query("INSERT into pomitra (idpomitra,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) values
                        (null,'$idmitramarketer','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total',
                        ...
                        $jmlh[$x]   = 0;
                        $total      = 0;
                        $sqlpo      = $koneksi->query("INSERT into pomitra (idpomitra,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) values
                        (null,'$idmitramarketer','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total','$invoice','Belum DP',NOW(),'$waktu')");
                    }
                }
