...
echo "
    ...
    <center><b>Anda sudah mengisi Formulir $namapo , Klik Tombol Dibawah ini Jika ingin melihat atau merevisi Invoice, 
      ...
      <a class='btn btn-info' href='datapom.php?invoice=M$idpoproduk-$idmitramarketer&id=$id'> INVOICE</a> </center>";
    }
?>
  </table>
  </div><br><br>
            ...
            $query = $koneksi->query($sql);
              while($row = $query->fetch_assoc()){
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="jmlh[]"  style="width:50px;" value='0'> <label>Pcs </label><br>
                   ...
                   $nobaru=$no+$ab;
                             ...
                             $jumlah_dipilih = count($jmlh);
                                ...
                                $sisa = $query->fetch_assoc();
                                             ...
                                             if($sisa['stok']>$jmlh[$x]){
                                      $koneksi->query("insert into pomitra (idpomitra,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl) values
                                     (null,'$idmitramarketer','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$custom[$x]','$total','M$idpoproduk-$idmitramarketer','Belum Acc DB',NOW())");
                                    ...
                                    // $koneksi->query("update pokategori set stok=stok-'$jmlh[$x]' where idpo='$idpo'");
                                      echo "<script>alert('data berhasil dikirim');</script>";
                                            echo "<script>location='datapom.php?invoice=M$idpoproduk-$idmitramarketer&id=$idpoproduk';</script>";
                                      }else{ 
                                          $jmlh[$x]=0;
                                          $total=0;
                                     $koneksi->query("insert into pomitra (idpomitra,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl) values
