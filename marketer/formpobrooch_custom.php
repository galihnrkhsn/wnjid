...
if(!isset($_SESSION["mitraagen"])){
 ...
 <title>Mitra <?php echo $_SESSION['mitramarketer']['namaagen']; ?>| WNJ </title>
<html lang="en">
<head>
  <meta charset="utf-8">
...
$query_variant = "SELECT podetail.harga, podetail.idpo
  ...
  if (preg_match("/^\S{1,}$/", $namanya)) {
    $sqlinsert = $koneksi->query("insert into pomitra (idpomitra,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl,waktu) values
    (null,'$idmitramarketer','$idpoproduk','$idpo','$idpodetail[$x]','$jumlah[$x]','$namanya','$total',
    'BM$idpoproduk-$idmitramarketer-$nobaru','Belum Acc DB',NOW(),'$waktu')");
...
if ($sqlinsert) {
  echo "<script>location='datapom.php?id=$idpoproduk&invoice=BM$idpoproduk-$idmitramarketer-$nobaru';</script>";  
}
