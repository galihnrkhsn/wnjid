                        ...
                        <?php if ($idmitramarketer == 981) : ?>
                            <div class="col-sm-12">
                                <div class="form-group my-1">
                                    ...
                                    <?php
                                        $query = $koneksi->query("SELECT * FROM mitramarketer ORDER BY namaagen ASC");
                                        while ($data = $query->fetch_assoc()) {
                                    ?>
                                        <option value="<?= $data['idmitramarketer'] ?>"><?= $data['namaagen'] ?> (<?= $data['idmitramarketer'] ?>)</option>
                                    <?php } ?>
                                    </select>
                                </div>
                            </div>
    ...
    <?php
        if (isset($_POST['kirim'])) {
            try {
                ...
                if ($idmitramarketer == 981) {
                    $iddb = $_POST['iddb'];
                } else {
                    $iddb = $_SESSION['idmitramarketer'];
                }
                ...
                if (isset($_GET['invoice'])) {
                    ...
                    if ($jenis_po === "Set") {
                        ...
                        $invoice = 'M' . $idpoproduk . '-' . $iddb . $angka_acak;
                    } else {
                        $invoice = 'M' . $idpoproduk . '-' . $iddb;
                    }
                }
                ...
                $query_check = $koneksi->query("SELECT COUNT(*) AS jumlah_invoice FROM pomitra WHERE invoice = '$invoice' AND idmitramarketer = '$iddb'");
                    ...
                    if ($jenis_po === "Set") {
                        ...
                        if ($total_item % 3 == 0) {
                            $sql = $koneksi->query("INSERT INTO pomitra
                                                    (idpomitra, idmitramarketer, idpoproduk,
