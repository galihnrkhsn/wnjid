<?php 
    ...
    $ambil  = $koneksi->query("SELECT COUNT(*) as jmlh FROM keranjang WHERE idproduk = '$idvariant' AND idmarketer = '$idmitramarketer' "); 
    $data   = $ambil->fetch_assoc();

    if($data['jmlh'] == '1' && $stock['stock'] > 0){
        $koneksi->query("UPDATE keranjang SET jmlh = jmlh + 1, subtotal = subtotal + '$harga' WHERE idproduk = '$idvariant' AND idmarketer = '$idmitramarketer' ");
        ...
        $koneksi->query("INSERT INTO keranjang (idkeranjang, idproduk, idmitra, idagen, idreseller, 
                                ...
                                VALUES (NULL, '$idvariant', '', '', '', '$idmitramarketer', '1', '$harga', 
