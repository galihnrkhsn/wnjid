<?php 
    ...
    $queryUser  = $koneksi->query("SELECT * FROM mitramarketer WHERE idmitramarketer = '$idadmin'");
    ...
    <body>
        ...
        <div class="container mx-auto mt-2 mb-3">
            ...
            <?php
                ...
                $sqlcheck2      = $koneksi->query("SELECT * FROM ordermarketer WHERE invoice = '$invoice'");
                ...
                if (!$pengirimancek) {
            ?>
                <p>Alamat belum di isi. Silahkan isi alamat <a class="link text-danger" href="formpengirimanb.php?id=<?= $invoice ?>&berat=<?= $beratnya ?>">disini.</a></p>
            <?php } ?>
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                            $no = 1;
                            $sql = $koneksi->query("SELECT * FROM ordermarketer
                        ...
                        ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><?= $order['namaproduk']; ?> <?= $order['variant'] ?></td>
                                <td><?= $order['jumlah']; ?></td>
                                <td>Rp. <?= number_format($order['subtotal']) ?></td>
                            </tr>
                        <?php
                                $total_order += $order['jumlah'];
                                $jenis_barang = $order['jenis'];
                            }

                            $total_harga = $total_order * 100000 / 4;
                ...
                <?php
                    ...
                    if ($jenis_barang === "Sale") {
                        $diskona = $total_harga * 35/100;
                        $tdiskona = number_format($diskona);
                        $tongkir = number_format($pengiriman['ongkir']);
                        if ($ekspedisi == "wahana") {
                            $grandtotal = ($total_harga + $pengiriman['ongkir'] + $biayad + $asuransi) - $diskona - $diskonpromo;
                        } else {
                            $grandtotal = ($total_harga + $pengiriman['ongkir'] + $biayad) - $diskona - $diskonpromo;
                        }
                    } else {
                        ...
                        $tongkir = number_format($pengiriman['ongkir']);
                        if ($ekspedisi == "wahana") {
                            $grandtotal = ($totala + $pengiriman['ongkir'] + $biayad + $asuransi) - $diskona - $diskonpromo;
                        } else {
                            $grandtotal = ($totala + $pengiriman['ongkir'] + $biayad) - $diskona - $diskonpromo;
                        }
                    }
