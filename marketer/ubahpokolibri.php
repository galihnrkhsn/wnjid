...
if(!isset($_SESSION["mitraagen"])){
 ...
 $invoice = $_GET['invoice'];
  ...
  $query = "SELECT COUNT(*) as jumlah,poproduk.idpoproduk,poproduk.namapo,poproduk.status FROM poproduk inner join pokolibri on poproduk.idpoproduk=pokolibri.idpoproduk WHERE pokolibri.invoice='$invoice'";
...
<!--================ NAVBARU END =================-->
		...
		<div class="container" align="center">
	        <?php        
			$sql = mysqli_query($koneksi, "SELECT * FROM podropship_kolibri WHERE invoice='$invoice'");
			while($data = mysqli_fetch_array($sql)){ 
			
			?>    
			<p align="center"><strong>SALES INVOICE</strong></p>
			<p align="center"><strong>PO Kolibri</strong></p><br>
			...
			<?php
					...
					$sql2 = mysqli_query($koneksi, "SELECT poproduk.namapo,pokategori.namakategori,podetail.variant,pokolibri.idpokolibri,pokolibri.jumlah,pokolibri.invoice,pokolibri.total,podetail.harga 
					FROM poproduk inner JOIN pokategori inner join podetail inner join pokolibri on poproduk.idpoproduk=pokolibri.idpoproduk and pokategori.idpo=pokolibri.idpo and podetail.idpodetail=pokolibri.idpodetail WHERE pokolibri.invoice='$invoice'");
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					$subtotal=0;
					// Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					$sql = mysqli_query($koneksi, "SELECT poproduk.namapo,pokategori.namakategori,podetail.variant,pokolibri.idpokolibri,pokolibri.jumlah,pokolibri.invoice,pokolibri.total,podetail.harga 
					FROM poproduk inner JOIN pokategori inner join podetail inner join pokolibri on poproduk.idpoproduk=pokolibri.idpoproduk and pokategori.idpo=pokolibri.idpo and podetail.idpodetail=pokolibri.idpodetail WHERE pokolibri.invoice='$invoice'");
					...
					?>
						<tr>
							...
							<form method="POST"><input type="hidden" name="harga" value=<?php echo $data['harga']; ?>>
							                    <input type="hidden" name="idpokolibri" value=<?php echo $data['idpokolibri']; ?>>
							                    <td class="align-middle"><input type="text" name="jmlh" size="1">
                                    	 ...
                                    	 //$jmlhtotal=count($total);
                                           // for($x=0;$x<$jmlhtotal;$x++){
                                            //echo number_format($total, 2);
                                              ...
                                              //$tot=$total;
                                              //$tot=0;
        					                   $koneksi->query("UPDATE pokolibri set jumlah='$jmlh', total='$total' where idpokolibri='$idpokolibri';");
        					                   	echo "<script>alert('Data berhasil diubah');</script>";
		                                        echo "<script>location='ubahpokolibri.php?invoice=$invoice';</script>";
        					                     }
                            ?>
					...
					?>
				            ...
				            <?php $diskon=15/100*$jumlah;
				                  $subtotal=$jumlah-$diskon; ?>
				            <p align="right">Diskon Reseller 15% Rp. <?php echo number_format($diskon); ?> </p><br>  
				            <p align="right">TOTAL  Rp. <?php echo number_format($subtotal); ?> </p>
				          
		                   <a class="btn btn-primary" href="datapokolibri.php?invoice=<?php echo $invoice ?>">Simpan</a>
