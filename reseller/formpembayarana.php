        ...
        if (empty($errors)) {
            ...
            if (move_uploaded_file($tmp, '../adminwnj/bukti/' . $namaFileBaru)) {
                ...
                try {
                    ...
                    $getRek = $koneksi->query("SELECT * FROM rekeningku WHERE idmitrareseller = '$idmitrareseller'");
                    $dataRek = $getRek->fetch_assoc();
    
                    if (!$dataRek) {
                        // Insert bank information if it does not exist
                        $insertBank = $koneksi->query("INSERT INTO rekeningku (idrek, idadmin, idmitraagen, idmitrareseller, idmitramarketer, bank, namapemilik, rekening)
                            VALUES (NULL, NULL, NULL, '$idmitrareseller', NULL, '$bankpengirim', '$namapengirim', '$rekeningpengirim')");
                    }
                    ...
                    // Update order status
                    $koneksi->query("UPDATE orderreseller SET status='Tunggu Confrim Admin', payment='Sudah Konfirmasi' WHERE invoice='$invoiceNilai'");
