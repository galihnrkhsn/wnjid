...
$ambil=$koneksi->query("SELECT COUNT(*) as jmlh FROM keranjangdb where idprodukdb='$idprodukdb' and idreseller='$idreseller'"); 
$data=$ambil->fetch_assoc();

        if($data['jmlh']=='1' and $stock['stock']>0){
            $koneksi->query("UPDATE keranjangdb SET jmlh=jmlh+1,subtotal=subtotal+'$harga' where idprodukdb='$idprodukdb' and idreseller='$idreseller'");
            ...
            $koneksi->query("INSERT INTO keranjangdb (idkeranjangdb,idprodukdb,idreseller,jmlh,harga,subtotal,tgl)
			VALUES (null,'$idprodukdb','$idreseller','1','$harga','$harga',NOW())");
			$koneksi->query("UPDATE produkdb SET stock=stock-1 where idprodukdb='$idprodukdb'");
