...
<input type="text" name="namaproduk">  <button type="submit" class="btn btn-primary" name="cari">Cari</button>  <button type="submit" class="btn btn-primary" name="tampil">Tampil Semua</button></form></pre>-->
    ...
    <!--          <input type="text" placeholder="Cari No Order..."  name="noorder"> -->
	  ...
	  $limit = 20; 
	  $limit_start = ($page - 1) * $limit;
      $ambil=$koneksi->query("SELECT DISTINCT orderagen.idmitra, orderpembayaran.tgl,orderpembayaran.rekeningpengirim,mitraagen.namamitra,orderpengiriman.invoice,orderpengiriman.total,orderpembayaran.bankpengirim,orderpembayaran.metodebayar,orderpembayaran.jmlhtransfer FROM orderpengiriman inner join mitraagen inner join orderpembayaran inner join orderagen on mitraagen.idmitraagen=orderagen.idmitra and orderpembayaran.invoice=orderpengiriman.invoice GROUP by orderpembayaran.invoice ORDER BY orderpembayaran.idpembayaran DESC LIMIT ".$limit_start.",".$limit." "); 
      }else{
      $no=1;
      $page = (isset($_GET['page']))? $_GET['page'] : 1;
	  $limit = 20; 
