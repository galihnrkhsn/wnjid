<?php
    ...
    $ambil              = $koneksi->query("SELECT count(*) as bank FROM rekeningku where idmitrareseller='$idmitrareseller'"); 
...
<body>
    ...
    <div class="container mt-5">   
        <h4 align="center">
            <?php echo $queryUser["namaagen"]; ?> (Cust ID : <?php echo $_SESSION["idmitrareseller"]; ?> )
        </h4>
                        ...
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <?php else: ?>
                                <div class='form-group'>
                                    ...
                                    <select name='bankpengirim' class='form-control'>
                                        <?php 
                                            $ambil=$koneksi->query("SELECT * FROM rekeningku where idmitrareseller='$idmitrareseller'"); 
                                            while($reseller=$ambil->fetch_assoc()):
                                        ?>
                                        <option value="<?php echo $reseller['bank']?>"><?php echo $reseller['bank'] ?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                            <?php endif; ?>
                                ...
                                <div class='form-group'>
                                    ...
                                    <select name='rekeningpengirim' class='form-control' required>
                                        <?php 
                                            ...
                                            while($reseller=$ambil->fetch_assoc()): 
                                        ?>
                                        <option value="<?php echo $reseller['namapemilik']." ".$reseller['rekening']?>"><?php echo $reseller['namapemilik']." ".$reseller['rekening']?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="metodebayar" required>
                                    ...
                                    <?php 
                                        $ambil_rek=$koneksi->query("SELECT * FROM rekeningwnj"); 
    ...
    <?php   
        if(isset($_POST['kirim'])){
            ...
            if ( $foto <> '' ) {
                ...
                $namaFileBaru = 'R' . $idmitrareseller . $idpoproduk . uniqid();
                $namaFileBaru.='.';
            ...
            if ( $jenis == 'dp' ) {
                ...
                try {
                    ...
                    $koneksi->query("UPDATE pomitra SET status = 'Sudah Confirm $jenis' WHERE invoice = '$invoice'");
                    if ($sql) {
                        echo "BERHASIL INPUT PAYMENT 2";
                    }
                } catch(Exception $error) {
                    echo "Konfirmasi Gagal! 2";
                    echo $error->getMessage();
                }
            } elseif ($jenis == "Payment 3") {
                try {
                    $sql = $koneksi->query("INSERT INTO popembayaran 
                                                    (
                                                        idpembayaran, idpoproduk, invoice,
                    ...
                    $koneksi->query("UPDATE pomitra SET status = 'Sudah Confirm $jenis' WHERE invoice = '$invoice'");
                    if ($sql) {
                        echo "BERHASIL INPUT PAYMENT 3";
                    }
                } catch(Exception $error) {
                    echo "Konfirmasi Gagal! 3";
                    echo $error->getMessage();
                }
            } elseif ($jenis == "Payment 4") {
                try {
                    $sql = $koneksi->query("INSERT INTO popembayaran 
                                                    (
                                                        idpembayaran, idpoproduk, invoice,
                    ...
                    $koneksi->query("UPDATE pomitra SET status = 'Sudah Confirm $jenis' WHERE invoice = '$invoice'");
                    if ($sql) {
                        echo "BERHASIL INPUT PAYMENT 4";
                    }
                } catch(Exception $error) {
                    echo "Konfirmasi Gagal! 4";
                    echo $error->getMessage();
                }
            } elseif ($jenis == "Payment 5") {
                try {
                    $sql = $koneksi->query("INSERT INTO popembayaran 
                                                    (
                                                        idpembayaran, idpoproduk, invoice,
