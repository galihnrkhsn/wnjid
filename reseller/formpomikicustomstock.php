...
echo "
    ...
    <center><b>Anda sudah mengisi Formulir $namapo , Klik Tombol Dibawah ini Jika ingin melihat atau merevisi Invoice, 
      ...
      <a class='btn btn-info' href='datapom.php?invoice=R$idpoproduk-$idmitrareseller&id=$id'> INVOICE</a> </center>";
    }
?>
  </table>
  </div><br><br>
                   ...
                   $nobaru=$no+$ab;
                             ...
                             $jumlah_dipilih = count($jmlh);
                                ...
                                $sisa = $query->fetch_assoc();
                                             ...
                                             if($sisa['stok']>$jmlh[$x]){
                                      $koneksi->query("insert into pomitra (idpomitra,idmitrareseller,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl) values
                                     (null,'$idmitrareseller','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$custom[$x]','$total','R$idpoproduk-$idmitrareseller','Belum Acc DB',NOW())");
                                    ...
                                    // $koneksi->query("update pokategori set stok=stok-'$jmlh[$x]' where idpo='$idpo'");
                                      echo "<script>alert('data berhasil dikirim');</script>";
                                            echo "<script>location='datapom.php?invoice=R$idpoproduk-$idmitrareseller&id=$idpoproduk';</script>";
                                      }else{ 
                                          $jmlh[$x]=0;
                                          $total=0;
                                     $koneksi->query("insert into pomitra (idpomitra,idmitrareseller,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl) values
