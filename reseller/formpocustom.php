                        ...
                        <?php if ($idmitrareseller == 899) : ?>
                            <div class="col-sm-12">
                                <div class="form-group my-1">
                                    ...
                                    <?php
                                        $query = $koneksi->query("SELECT * FROM mitrareseller ORDER BY namaagen ASC");
                                        while ($data = $query->fetch_assoc()) {
                                    ?>
                                        <option value="<?= $data['idmitrareseller'] ?>"><?= $data['namaagen'] ?> (<?= $data['idmitrareseller'] ?>)</option>
                                    <?php } ?>
                                    </select>
                                </div>
                            </div>
    ...
    <?php
        if (isset($_POST['kirim'])) {
            try {
                ...
                if ($idmitrareseller == 899) {
                    $iddb = $_POST['iddb'];
                } else {
                    $iddb = $_SESSION['idmitrareseller'];
                }
                ...
                if (isset($_GET['invoice'])) {
                    ...
                    if ($jenis_po === "Set") {
                        ...
                        $invoice = 'R' . $idpoproduk . '-' . $iddb . $angka_acak;
                    } else {
                        $invoice = 'R' . $idpoproduk . '-' . $iddb;
                    }
                }
                ...
                $query_check = $koneksi->query("SELECT COUNT(*) AS jumlah_invoice FROM pomitra WHERE invoice = '$invoice' AND idmitrareseller = '$iddb'");
                    ...
                    if ($jenis_po === "Set") {
                        ...
                        if ($total_item % 3 == 0) {
                            $sql = $koneksi->query("INSERT INTO pomitra
                                                    (idpomitra, idmitrareseller, idpoproduk,
