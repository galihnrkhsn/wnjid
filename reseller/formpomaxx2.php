...
echo "
    ...
    <center><b>Anda sudah mengisi Formulir $namapo , Klik Tombol Dibawah ini Jika ingin melihat atau merevisi Invoice, 
      ...
      <a class='btn btn-info' href='datapomax.php?idmitra=$idmitrareseller&id=$id'> INVOICE</a> </center>";
    }
?>
  </table>
  </div><br><br>
	...
	<?php $stok= $stok['stok']; } ?>
		...
		<div style="padding: 0 15px;">
					...
					<?php
						...
						$total = 0;
						$index = 0;
						$sql = "SELECT * FROM poproduk inner join pokategori inner join podetail on poproduk.idpoproduk=pokategori.idpoproduk and pokategori.idpo=podetail.idpo where poproduk.idpoproduk='$idpoproduk' and podetail.idpodetail>=2794 order by podetail.idpodetail desc ";
						$query = $koneksi->query($sql);
							while($row = $query->fetch_assoc()){
								...
								<div class="form-group">
								    ...
								    <input type="text" name="jmlh[]" class="form-control" style="width:80px;" value='0'>
                                    	 ...
                                    	 //$jmlhtotal=count($total);
                                           // for($x=0;$x<$jmlhtotal;$x++){
                                            //echo number_format($total, 2);
                                             ...
                                             if($sisa['stok']>$jmlh[$x]){
        					                   $koneksi->query("insert into pomaximus (idpomaximus,idadmin,idmitraagen,idmitrareseller,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl) values
        					                   (null,'0','0','$idmitrareseller','0','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total','R$idpoproduk-$idmitrareseller','Belum Acc DB',NOW())");
        					                   $koneksi->query("update pokategori set stok=stok-'$jmlh[$x]' where idpo='$idpo[$x]'");
        					                   	echo "<script>alert('data berhasil dikirim');</script>";
		                                        echo "<script>location='datapomax.php?id=$idpoproduk';</script>";
        					                    }else{ 
        					                        $jmlh[$x]=0;
        					                        $total=0;
        					                   $koneksi->query("insert into pomaximus (idpomaximus,idadmin,idmitraagen,idmitrareseller,idmitramarketer,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl) values
        					                   (null,'0','0','$idmitrareseller','0','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total','R$idpoproduk-$idmitrareseller','Belum DP',NOW())");
