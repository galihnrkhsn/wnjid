<?php 
	...
	$provinsi	= $dataReseller['provinsi'];
	$kabupaten	= $dataReseller['kota'];
	$kecamatan	= $dataReseller['kecamatan'];
?>
					...
					<div class="col-md-6">
						...
						<form method="post">
							<div class="form-group">
								...
								<input type="text" class="form-control" name="namapengirim" value="<?= $dataReseller['namaagen']; ?>" required>
							</div>
							<div class="form-group">
								<label>Telepon Pengirim</label>
								<input type="text" class="form-control" name="tlppengirim" value="<?= $dataReseller['whatsapp']; ?>" required maxlength="17">
							</div>
							<hr>
							<div class="form-group">
								<label>Nama Penerima</label>
								<input type="text" class="form-control" name="namapenerima" value="<?= $dataReseller['namaagen']; ?>" required>
							</div>
							<div class="form-group">
								<label>Telepon Penerima</label>
								<input type="text" class="form-control" name="tlppenerima" value="<?= $dataReseller['whatsapp']; ?>" required maxlength="17">
							</div>
							<div class="form-group">
								<label>Alamat</label>
								<textarea class="form-control" name="alamat" required><?= $dataReseller['alamat']; ?></textarea>
							</div>
							<div class="form-group">
								<label>Kode POS</label>
								<input type="text" class="form-control" name="kodepos" value="<?= $dataReseller['kodepos']; ?>" required>
							</div>
							<div class="form-group">
								<label for="prov">Provinsi Tujuan</label><br>
								<select class="form-control" id="prov" name="prov" required>
									<?php
										$idprov	= $dataReseller["provinsi"];
										if($idprov==''){
...
<?php
	if(isset($_POST['kirim'])){
		...
		$sql 		= "SELECT SUM(orderreseller.subtotal) AS subtotal, SUM(orderreseller.jumlah) AS jumlah, variants.jenis 
						FROM orderreseller 
		...
		if ($pengirimancek) {
			...
			$query 		 = "INSERT INTO orderpengiriman (idorderp, namapengirim, tlppengirim, namapenerima, tlppenerima, alamat, provinsi, kota, kecamatan,
													ekspedisi, layanan, berat, ongkir, dropship, kodepos, invoice, total, diskonramadhan, tgl) 
							VALUES (NULL, '$namapengirim', '$tlppengirim', '$namapenerima', '$tlppenerima', '$alamat', '$provinsi', '$kabupaten',
											'$kecamatan', '$ekspedisi', '$layananku', '$berat', '$ongkir', 'ya', '$kodepos', '$invoice', 
											'$total', 0, NOW())"; 
			$sql 		 = mysqli_query( $koneksi, $query ); 
		} else{
			$query 		 = "INSERT INTO orderpengiriman (idorderp, namapengirim, tlppengirim, namapenerima, tlppenerima, alamat, provinsi, kota, 
													kecamatan, ekspedisi, layanan, berat, ongkir, dropship, kodepos, invoice, 
													total, diskonramadhan, tgl) 
							VALUE (NULL, '$namapengirim', '$tlppengirim', '$namapenerima', '$tlppenerima', '$alamat', '$provinsi', '$kabupaten', 
											'$kecamatan', '$ekspedisi', '$layananku', '$berat', '$ongkir', 'ya', '$kodepos', '$invoice', 
											'$total', 0, NOW())";
			$sql 		 = mysqli_query( $koneksi, $query); 	
		}  
