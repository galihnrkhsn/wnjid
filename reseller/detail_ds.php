...
<head>
    ...
    <meta name="author" content=""><title>Reseller | WNJ.ID</title>
</head>
<body>
    <!-- NAVBAR -->
 ...
 $sisa ?>">
        ...
        <? 
            $invocenya  = $data_ds['invoice'];
            $queryPay   = $koneksi->query("SELECT status FROM pomitra WHERE invoice = '$invocenya'");
            $dataPay    = $queryPay->fetch_assoc();
            if ($dataPay['status'] == 'Belum DP') :
        ?>
        ...
        <form method="POST" enctype="multipart/form-data">
            <div id="variantContainer">
                <div class="row mx-0" id="variantForm_0">
                    <div class="col-12">
                        <h6 class="text-uppercase text-danger">Variant 1</h6>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                                    ...
                                    if ($ambil) {
                                        while ($data = $ambil->fetch_assoc()) {
                                            ...
                                            <option value="<?= $data['idpodetail']; ?>" data-stok="<?= $data['stok']; ?>">
                                                <?= $data['variant']; $data['stok']?><?= !empty($custom) ? " | $custom" : ""; ?>
                                            </option>
                                            <?php
                                        }
                                    } else {
                    ...
                    foreach ($_POST['hiddenProductInput'] as $key => $index) {
                        ...
                        if ($jumlah > 0) {
                            ...
                            try {
                                ...
                                $koneksi->rollback();
                                echo $e->getMessage();
                                echo "<script>alert('Jumlah yang dimasukkan untuk varian $variant melebihi stok yang tersedia ($stokTersedia).')</script>";
    ...
    <script type="text/javascript">
        ...
        buttonAddVariant.addEventListener('click', function() {
            ...
            variantForm.innerHTML = `
                <div class="col-12">
                    <h6 class="text-uppercase text-danger">Variant ${variantFormCount + 1}</h6>
                </div>
                <div class="col-sm-4">
                    <div class="form-group">
                        <label for="variant" class="d-flex align-items-center">Produk<p class="text-danger p-0 m-0 ml-1">*</p></label>
