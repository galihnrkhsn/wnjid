...
$query_variant = "SELECT podetail.harga, podetail.idpo
  ...
  if (preg_match("/^\S{1,}$/", $namanya)) {
    $sqlinsert = $koneksi->query("insert into pomitra (idpomitra,idmitrareseller,idpoproduk,idpo,idpodetail,jumlah,custom,total,invoice,status,tgl,waktu) values
    (null,'$idmitrareseller','$idpoproduk','$idpo','$idpodetail[$x]','$jumlah[$x]','$namanya','$total',
    'BR$idpoproduk-$idmitrareseller-$nobaru','Belum Acc DB',NOW(),'$waktu')");
...
if ($sqlinsert) {
  echo "<script>location='datapom.php?id=$idpoproduk&invoice=BR$idpoproduk-$idmitrareseller-$nobaru';</script>";  
}
