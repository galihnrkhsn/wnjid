		...
		$total=0;
			...
			for($x=0;$x<$jumlah_dipilih;$x++){
				...
				$sisa = $query_sisa->fetch_assoc();
					...
					if($sisa['stok']>$jmlh[$x]){
						$koneksi->query("INSERT into pomitra (idpomitra,idmitrareseller,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) 
							values (null,'$idmitrareseller','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total','R$idpoproduk-$idmitrareseller','Belum Acc DB',NOW(),'$today')");
