									...
									<td>
										Inv :
											...
											<?php elseif ($data['jenis'] == 'Flash'): ?>
												<a class="text-primary" href="detailorder2.php?id=<?= $data['invoice']; ?>&jenis=<?= $data['jenis']; ?>">#<?= $data['invoice'] ?></a>
											...
											<?php elseif (strtotime($data['tgl']) > strtotime('2024-10-02 23:59:59') && substr($data['invoice'], 0, 2) == "RP"): ?>
											...
											<?php elseif (substr($data['invoice'], 0, 2) == "RP") : ?>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="tb_datareseller">
							...
							<?php
								...
								$sql = mysqli_query($koneksi, "SELECT DISTINCT mitrareseller.namaagen, orderreseller.invoice, orderreseller.payment, 
																orderreseller.status,orderreseller.tgl, admin_mitra.namamitra
														FROM `orderreseller` 
														INNER JOIN mitrareseller 
														ON mitrareseller.idmitrareseller = orderreseller.idmitrareseller 
                                                        INNER JOIN admin_mitra
                                                        ON admin_mitra.idadmin = mitrareseller.idadmin
														WHERE orderreseller.idmitrareseller='$idmitra' 
														AND (orderreseller.payment = 'Lunas' OR orderreseller.payment='LUNAS') 
