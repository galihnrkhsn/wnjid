...
if(!isset($_SESSION["mitraagen"])){
  ...
  $query = "SELECT COUNT(*) as jumlah,poproduk.idpoproduk,poproduk.namapo,poproduk.status FROM poproduk inner join poagen on poproduk.idpoproduk=poagen.idpoproduk WHERE poproduk.idpoproduk='$idpoproduk' AND poagen.idreseller='$idmitrareseller'";
...
echo "
    ...
    <center><b>Anda sudah mengisi Formulir $namapo , Klik Tombol Dibawah ini Jika ingin melihat atau merevisi Invoice, 
      ...
      <a class='btn btn-info' href='datapo.php?poagen=$idmitrareseller&id=$id'> INVOICE</a> </center>";
    }
?>
  </table>
  </div><br><br>
			                       ...
			                       <?php
			                        if(isset($_POST["save"])){
                                        ...
                                        for($x=0;$x<$jumlah_dipilih;$x++){
                                            ...
                                            $tot=0;
                                            // if($stok>$jmlh[$x]){
        					                $koneksi->query("insert into poagen (idpoagen,idreseller,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl) values
        					                ('null','$idmitrareseller','$idpoproduk','$idpo[$x]','$idpodetail[$x]','$jmlh[$x]','$total','R$idpoproduk-$idmitrareseller','Belum ACC DB',NOW())");
