...
if(!isset($_SESSION["mitraagen"])){
 ...
 $idpoproduk = $_GET['id'];
  ...
  $query = "SELECT COUNT(*) as jumlah,poproduk.idpoproduk,poproduk.namapo,poproduk.status FROM poproduk inner join pomaximus on poproduk.idpoproduk=pomaximus.idpoproduk WHERE poproduk.idpoproduk='$idpoproduk' AND pomaximus.idmitrareseller='$idmitrareseller'";
...
<input type="text" name="namaproduk">  <button type="submit" class="btn btn-primary" name="cari">Cari</button>  <button type="submit" class="btn btn-primary" name="tampil">Tampil Semua</button></form></pre>-->  
			...
			<?php
					...
					FROM poproduk inner JOIN pokategori inner join podetail inner join pomaximus on poproduk.idpoproduk=pomaximus.idpoproduk and pokategori.idpo=pomaximus.idpo and podetail.idpodetail=pomaximus.idpodetail WHERE pomaximus.idmitrareseller='$idmitrareseller' and pomaximus.idpoproduk='$idpoproduk'");
	...
	<?php $stok= $stok['stok']; } ?>
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					$subtotal=0;
					// Buat query untuk menampilkan data siswa sesuai limit yang ditentukan
					$sql = mysqli_query($koneksi, "SELECT poproduk.namapo,pokategori.namakategori,podetail.variant,pomaximus.idpomaximus,pomaximus.idpo,pomaximus.jumlah,pomaximus.invoice,pomaximus.total,podetail.harga 
					FROM poproduk inner JOIN pokategori inner join podetail inner join pomaximus on poproduk.idpoproduk=pomaximus.idpoproduk and pokategori.idpo=pomaximus.idpo and podetail.idpodetail=pomaximus.idpodetail WHERE pomaximus.idmitrareseller='$idmitrareseller' and pomaximus.idpoproduk='$idpoproduk'");
					...
					?>
						<tr>
							...
							<form method="POST"><input type="hidden" name="harga" value=<?php echo $data['harga']; ?>>
							                    ...
							                    <input type="hidden" name="idpomaximus" value=<?php echo $data['idpomaximus']; ?>>
							                    <td class="align-middle"><input type="text" name="jmlh" size="1"> <button type="submit" class="btn btn-primary" name="tambah">+</button>  <button type="submit" class="btn btn-danger" name="kurang">-</button>
							                   </td>
		                    </form>
					...
					?>
                                 ...
                                 $idpomaximus= $_POST['idpomaximus'];
			                       ...
			                       $total=$jmlh*$harga;
			                        ...
			                        $sql = "Select stok from pokategori where idpo='$idpo'";
					                          ...
					                          if($sisa['stok']>=$jmlh){
        					                   $koneksi->query("UPDATE pomaximus set jumlah=jumlah+'$jmlh' where idpomaximus='$idpomaximus';");
        					                   $koneksi->query("UPDATE pomaximus set total=jumlah*'$harga' where idpomaximus='$idpomaximus';");
        					                   $koneksi->query("UPDATE pokategori set stok=stok-'$jmlh' where idpo='$idpo'");
        					                   	echo "<script>alert('data berhasil diubah');</script>";
		                                        echo "<script>location='ubahpomax.php?id=$idpoproduk';</script>";
        					                     }
        					                     else{
		                   ...
		                   <a class="btn btn-primary" href="datapomax.php?id=<?php echo $idpoproduk ?>">Simpan</a>
