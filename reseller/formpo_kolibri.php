...
if($datacocok>=1){
      $sqlnya = $koneksi->query("UPDATE pomitra set jumlah=jumlah + '$jmlh[$x]' WHERE idpodetail='$idpodetail[$x]' and invoice ='$invoice'");
        ...
        $sql = $koneksi->query("INSERT INTO pomitra (idpomitra,idmitrareseller,idpoproduk,idpo,idpodetail,jumlah,total,invoice,status,tgl,waktu) values
        (null,'$idadmin','$idpoproduk','$idpo','$idpodetail[$x]','$jmlh[$x]','$total','$invoice','Belum Acc DB',NOW(),'$waktu')");  
}
         ...
         if ($sql) {
            ...
            echo "<script>location='formpo_kolibri?id=$idpoproduk';</script>";
         }
