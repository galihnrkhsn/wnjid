...
include "koneksi.php";
	...
	if(isset($_POST['kirim'])){
	 ...
	 $sql = mysqli_query($koneksi, "SELECT iddropship FROM podropship_max order by iddropship desc limit 1");
            ...
            (null,'0','0','$idmitrareseller','0','$idpoproduk','MXR$idmitrareseller-$nobaru','Pending','$namapengirim','$tlppengirim','$namapenerima','$tlppenerima','$alamatpenerima','$keterangan','$ekspedisi')";
          $sql = mysqli_query( $koneksi, $query);  
		
		            if ( $sql == TRUE ){
					echo "<script>alert('Berhasil Disimpan, Silahkan input PO yang diinginkan');</script>";
						echo "<script>location='formpomaxx.php?id=51&inv=MXr$idmitrareseller-$nobaru'</script>";
				}else{
