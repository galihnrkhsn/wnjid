...
<form method="post">  
    ...
    <div class="form-group">
          ...
          <select class="form-control" name="idpoproduk" id="idpoproduk" required>
              ...
              ?>
          <option value="<?php echo $tampilkan['idpoproduk']; ?>|DP"><?php echo $tampilkan['namapo']; ?></option>
          <?php } ?>                       
          </select>      
    </div>
...
<br>  
  <br>
<button type="submit" name="but_save_po" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Bayar DP</button>
...
$sqlnya = $koneksi->query("INSERT INTO popembayaran (idpembayaran,idpoproduk,invoice,bankpengirim,rekeningpengirim,jmlhtransfer,metodebayar,jenis,tgl,waktu) 
                VALUES (null,'$idpoproduk','$invoicenya','$bank','$norek','$jmlh','$metodebayar','dp','$tgl','$waktu')");                        
        
                if ($sqlnya) {

                  if ($metodebayar=="Deposit") {
$koneksi->query("INSERT INTO saldo (id_saldo,idadmin,tgl,transaksi,debit,credit)
                VALUES (null,'$idadminnya','$tgl','Bayar Pelunasan $banknya $hari $bulan $namapo #$invoicenya (Deposit)','0','$totalbayar')");
                                    }
                                    else{
$koneksi->query("INSERT INTO saldo (id_saldo,idadmin,tgl,transaksi,debit,credit)
                VALUES (null,'$idadminnya','$tgl','Invoice $namapo #$invoicenya','0','$totalbayar')");
$koneksi->query("INSERT INTO saldo (id_saldo,idadmin,tgl,transaksi,debit,credit)
                VALUES (null,'$idadminnya','$tgl','Bayar DP $banknya $hari $bulan $namapo #$invoicenya','$jmlh','0')");                                                      
                                    }                  
...
$koneksi->query("UPDATE pomitra set status='$status' WHERE invoice='$invoicenya';"); 
                  ...
                  echo "<script>location='bayardp.php';</script>";  
                  }else{
