<?php 
    ...
    $datam      = $koneksi->query("SELECT admin_mitra.namamitra, admin_mitra.idadmin 
                                    FROM ordermitra 
...
<br>
<div class="container">
    <center><h3>Invoice #<?= $invoice; ?></h3></center><br>
    <div class="row align-items-start">
        <div class="col">
            <h5>Nama DB : <?= $tampilkanm['namamitra']; ?></h5>
        </div>
        <div class="col"></div>
        <div class="col">
            <h5>CS : <?= $tampilnama['namacs']; ?></h5>
        </div>
    </div>            
            ...
            <table class="table">
                ...
                <tbody>
                    <?php 
                        ...
                        $datapo=$koneksi->query("SELECT admin_mitra.namamitra, products.namaproduk, ordermitra.idorder, ordermitra.idproduk,
                                                        ordermitra.invoice, ordermitra.harga, SUM(ordermitra.jumlah) as jumlah, 
                                                        SUM(ordermitra.subtotal) as subtotal, variants.variant, variants.size 
                                                    FROM ordermitra
                                                    ...
                                                    GROUP BY variants.idproducts");

                        } else {                          
                            $datapo = $koneksi->query("SELECT admin_mitra.namamitra, products.namaproduk, ordermitra.idorder, ordermitra.invoice,
                                                            ordermitra.harga, ordermitra.jumlah, ordermitra.subtotal, variants.variant, variants.size 
                                                        FROM ordermitra 
                                                        INNER JOIN admin_mitra ON ordermitra.idmitra = admin_mitra.idadmin
