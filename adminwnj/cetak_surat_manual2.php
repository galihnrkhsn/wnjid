        ...
        <tbody>
            <?php 
                if ($statusnya == 'RS' or $statusnya == 'INV') {               
                    $datapo=$koneksi->query("SELECT products.namaproduk, surat_jalan_manual.progres, surat_jalan_manual.waktu, variants.variant, variants.size
                                                FROM surat_jalan_manual
                                                JOIN variants on variants.id = surat_jalan_manual.idproduk
                                                JOIN products ON products.id = variants.idproducts
                                                WHERE surat_jalan_manual.no_sj = '$no_sj' 
                                                AND surat_jalan_manual.progres > 0
                                            ");
                }
                if ($statusnya == 'PO' or $statusnya == 'IPO') {     
                    $datapo = $koneksi->query("SELECT podetail.variant as namaproduk, podetail.harga, surat_jalan_manual.progres, variants.variant, variants.size
                                                FROM surat_jalan_manual
