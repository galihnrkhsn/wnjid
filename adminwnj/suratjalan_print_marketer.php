        ...
        <h2>
            <strong>
                <?php 
                    ...
                    $tglnya     = date("dm");
                    $waktunya   = date("His");
                    $no_sj      = 'SJ-M'.$idmitramarketer.'-'.$tglnya.'-'.$waktunya;
                    echo $tgl; 
                    echo "<br>";
                    echo "No. ".$no_sj;
                ?>
    ...
    <?php 
        if(isset($_POST['but_export'])){ 
            $datamitra  = $koneksi->query("SELECT admin_mitra.idadmin,admin_mitra.namamitra, admin_mitra_cs.namacs,
                                                mitramarketer.namaagen, mitramarketer.idmitramarketer as idsubmitra 
                                            FROM mitramarketer
        ...
        <tbody>
            ...
            <?php 
                $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, products.namaproduk, ordermarketer.jumlah,
                                                ...
                                                surat_jalan_subdb.id_sj, ordermarketer.idmitramarketer, variants.variant, variants.size, 
                                                ordermarketer.tgl
            ...
            <?php 
                ...
                $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, produk.namaproduk, ordermarketer.jumlah,
                                                surat_jalan_subdb.progres, surat_jalan_subdb.status, surat_jalan_subdb.waktu,
                                                surat_jalan_subdb.id_sj, ordermarketer.idmitramarketer
                                            FROM surat_jalan_subdb
