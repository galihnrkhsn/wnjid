<?php 
    ...
    $datamitra  = $koneksi->query("SELECT namacs FROM admin_mitra_cs WHERE idadmin='$idadminya'");
					...
					<tr>
                      ...
                      <tbody>
                          ...
                          if(isset($_POST["cari"])){
                           ...
                           $page    =isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
                           $mulai    =($page>1) ? ($page * $halaman) - $halaman : 0;
                            $datasaldo=$koneksi->query("SELECT mitrareseller.namaagen,variants.variant,variants.size,products.namaproduk,orderreseller.invoice,orderreseller.idorder,orderreseller.harga,orderreseller.jumlah,orderreseller.subtotal 
                                                        FROM orderreseller 
                            ...
                            $datapo=$koneksi->query("SELECT mitrareseller.namaagen,variants.variant,variants.size,products.namaproduk,orderreseller.idorder,orderreseller.invoice,orderreseller.harga,orderreseller.jumlah,orderreseller.subtotal 
                                                        FROM orderreseller 
                                                        INNER JOIN mitrareseller ON orderreseller.idmitrareseller = mitrareseller.idmitrareseller
                                                        INNER JOIN variants on orderreseller.idproduk = variants.id
                                                        INNER JOIN products ON products.id = variants.idproducts 
