<?php 
    ...
    $queryOrder         = $koneksi->query("SELECT * FROM orderreseller WHERE idmitrareseller = '$idmitrareseller'");
...
<div class="tab-content">
    <div id="home" class="tab-pane fade show active" id="home"  role="tabpanel">    
        <div class="table-responsive">
            ...
            <form method="post" action="suratjalan_print_reseller.php" target="_blank">  
                    ...
                    <tbody>
                        <?php
                            $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, produk.namaproduk, orderreseller.jumlah,
                                                            ...
                                                            surat_jalan_subdb.id_sj, surat_jalan_subdb.idorder, orderreseller.idmitrareseller
                                                        ...
                                                        WHERE orderreseller.idmitrareseller = '$idmitrareseller' 
                        ...
                        ?>
                            <tr>
                                <td><input type='checkbox' name='update[]' value='<?= $id ?>' >
                                    ...
                                    <input type='hidden' name='idmitrareseller' value='<?= $tampilkan['idmitrareseller']; ?>' >
                                </td>
                                <td><?= $no++; ?></td>     
        ...
        <div class="table-responsive">
            ...
            <form method="post" action="suratjalan_status_subdb.php" >  
                <table class="table table-striped" id="tb_multiprint" target="_blank">
                    ...
                    <tbody>
                        <?php
                            if (strtorime($tgl) > strtotime('2024-11-02')) {
                                $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, products.namaproduk, orderreseller.jumlah,
                                                            surat_jalan_subdb.no_sj, surat_jalan_subdb.progres,
                                                            SUM(surat_jalan_subdb.progres) as jumlahnya, surat_jalan_subdb.status,
                                                            surat_jalan_subdb.waktu, surat_jalan_subdb.id_sj, orderreseller.idmitrareseller
                                                        FROM surat_jalan_subdb
                                                        JOIN variants on variants.id = surat_jalan_subdb.idproduk
                                                        JOIN products on products.id = variants.idproducts
                                                        JOIN orderreseller on (orderreseller.idorder = surat_jalan_subdb.idorder and orderreseller.invoice = surat_jalan_subdb.invoice)
                                                        WHERE orderreseller.idmitrareseller = '$idmitrareseller' and surat_jalan_subdb.progres>0 and surat_jalan_subdb.status = 'Checker'
                            ...
                            ?>
                                <tr>
                                    <td>
                                        ...
                                        <input type='hidden' name='id_sj<?= $id ?>' value='<?= $tampilkan['id_sj']; ?>' >
                                        <input type='hidden' name='idorder<?= $id ?>' value='<?= $tampilkan['idorder']; ?>' >
                                        <input type='hidden' name='idadmin' value='<?= $tampilkan['idmitrareseller']; ?>' >
                                    </td>
                                    <td><?= $no++; ?></td>  
                                    <td><a href="suratjalan_print_subdb.php?no_sj=<?= $tampilkan['no_sj'] ?>&idadmin=<?= $tampilkan['idmitrareseller'] ?>&jenis=Reseller" target="_blank"><i class="fa fa-print"></i> <?= $tampilkan['no_sj']; ?></a></td>   
                                    <td><?= $tampilkan['invoice']; ?></td>
                ...
                <input type='submit' class="btn btn-success" value='Ubah Status' name='but_update_reseller' onclick="return confirm('Yakin Akan Ubah Data?');">        
            </form>
        </div>
    </div>
</div>
