<?php
    ...
    $stmtCtx = $koneksi->prepare("SELECT status FROM ordermitra WHERE invoice = ? LIMIT 1");
    ...
    try {
        $stmtExisting = $koneksi->prepare("SELECT SUM(jumlah) AS total_jumlah FROM ordermitra WHERE invoice = ? AND idproduk = ? FOR UPDATE");
        ...
        $stmtCount = $koneksi->prepare("SELECT COUNT(DISTINCT idproduk) AS jumlah_item FROM ordermitra WHERE invoice = ?");
