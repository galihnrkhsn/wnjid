				...
				<table class="table table-bordered">
					<tr>
                      ...
                      <tbody>
                          <?php 
                            $subtotal=0;
                            $datapo=$koneksi->query("SELECT podetail.variant, sum(pomitra.jumlah) as jumlah, REPLACE(right(podetail.variant,2),' ', '') as ukuran FROM poproduk INNER JOIN pomitra INNER JOIN pokategori INNER JOIN podetail on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail where poproduk.idpoproduk='$id' and pomitra.status<>'Belum Acc DB' GROUP by podetail.variant order by ukuran");
                            $no=1;
					...
					<tr>
                      ...
                      <tbody>
                          <?php 
                            ...
                            $datapo2=$koneksi->query("SELECT pokategori.namakategori, sum(pomitra.jumlah) as jumlah FROM poproduk INNER JOIN pomitra INNER JOIN pokategori INNER JOIN podetail on poproduk.idpoproduk=pomitra.idpoproduk and pokategori.idpo=pomitra.idpo and podetail.idpodetail=pomitra.idpodetail where poproduk.idpoproduk='$id' and pomitra.status<>'Belum Acc DB' GROUP by pokategori.namakategori");
                            $no2=1;
