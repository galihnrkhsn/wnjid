<?php
    ...
    // Hanya admin tertentu yang boleh mengubah isi order (sama seperti detailorder2.php).
    ...
    // ---- Ambil item order utk ditampilkan ----
    $items     = [];
    $namamitra = null;
    ...
    $datapo = $koneksi->query("SELECT
                                ...
                                INNER JOIN variants ON orderagen.idproduk = variants.id
    ...
    while ($tampilkan = $datapo->fetch_assoc()) {
        ...
        $items[]          = $tampilkan;
        $namamitra        = $tampilkan['namaagen'];
    }

    $statusOrder = $items ? ($koneksi->query("SELECT status FROM orderagen WHERE invoice = '$invoice' LIMIT 1")->fetch_assoc()['status'] ?? '') : '';
    $editAktif   = $bisaEdit && in_array($statusOrder, $statusBisaEdit, true);

    // ================================================================================
    // PERHITUNGAN DISKON - LOGIKA ASLI, TIDAK DIUBAH (diskon dasar Agen 25%, beda dari
    // distributor yang 35% - ini memang aturan bisnis yang berbeda, bukan salah ketik).
    // Cuma tampilannya di-refactor & tier/dropship dipindah ke includes/discount_rules.php.
    // ================================================================================
    $totala = 0;
    $sql    = "SELECT * FROM orderagen
                ...
                AND products.idkategori > 0 AND products.idkategori <> 2";
    $query  = $koneksi->query($sql);
    ...
    // sumber yang sama dipakai detailorder2.php & distributor/detailorderb2.php.
    $totalPerCategory = [];
    ...
    // Bug lama: "if ($jenis = 'Flash')" (assignment, bukan perbandingan) bikin baris ini
    // SELALU tampil apa pun jenisnya. Dibenerin jadi "==" di sini.
    $diskonFlash = ($jenis == 'Flash') ? $totala * 20 / 100 : 0;

    $diskonLines = [];
    if ($diskona > 0)     $diskonLines[] = ['Diskon Agen 25%', $diskona];
    ...
    $grandtotal = ($totala + $totalb + $ongkir + $biayad)
                - ($diskona + $diskonFlash + $diskonb + $diskonKategoriTotal);
    $total      = $totala + $totalb;
...
<body id="page-top" class="sidebar-toggled">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-3">
            ...
            <a class="btn btn-success" href="cetakorderagen2.php?invoice=<?= urlencode($invoice) ?>" target="_blank">
            ...
            <div class="col-lg-4">
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <?php if ($diskonramadhan > 0): ?>
                        <div class="ringkasan-line diskon">
                            <span>Diskon Ramadhan</span>
                            <span>- Rp <?= number_format($diskonramadhan) ?></span>
                        </div>
                        <?php endif; ?>

                        <div class="ringkasan-line grandtotal">
                            <span>Grand Total</span>
                            <span>Rp <?= number_format($grandtotal - $diskonramadhan) ?></span>
                        </div>
                    </div>
                </div>
            </div>
  ...
  <script>
    ...
    $(document).on('click', '.btn-tambah-item', function () {
        ...
        $.post('api/detailorderagen_add_item.php', { invoice: INVOICE, idproduk: idproduk, jumlah: jumlah }, function (res) {
            if (res.success) {
                location.reload();
            } else {
    ...
    $(document).on('click', '.btn-simpan-jumlah', function () {
        ...
        $.post('api/detailorderagen_update_qty.php', { invoice: INVOICE, idproduk: idproduk, jumlah: jumlah }, function (res) {
            if (res.success) {
                location.reload();
            } else {
    ...
    $(document).on('click', '.btn-hapus-item', function () {
        ...
        $.post('api/detailorderagen_remove_item.php', { invoice: INVOICE, idproduk: idproduk }, function (res) {
            if (res.success) {
                location.reload();
            } else {
