 ...
 <?php include "sidebar.php"; ?>
   ...
   <h3><strong>Invoice <?php echo $invoice ?></strong></h3><br>
          ...
          <div class="row">
           ...
           <a class="btn btn-success" href="cetakorderagen2.php?invoice=<?php echo $invoice; ?>" target="blank">Cetak</a> &nbsp&nbsp&nbsp 
           <!-- <a class="btn btn-primary" href="cetak_suratjalan_agen.php?invoice=<?php echo $invoice; ?>" target="blank">Cetak SJ</a>     -->
					...
					<tr>
                      ...
                      <tbody>
                          ...
                          if(isset($_POST["cari"])){
                           ...
                           $page    =isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
                           $mulai    =($page>1) ? ($page * $halaman) - $halaman : 0;
                            $datasaldo=$koneksi->query("SELECT mitraagen.namaagen,products.namaproduk,variants.variant,
                                                        variants.size, orderagen.invoice, orderagen.idorder, orderagen.harga,
                                                        orderagen.jumlah, orderagen.subtotal 
                                                        FROM orderagen 
                            ...
                            $total = mysqli_num_rows($datasaldo);
                            $pages = ceil($total/$halaman);
                            $datapo=$koneksi->query("SELECT mitraagen.namaagen, products.namaproduk, variants.variant,
                                                        variants.size, variants.id, orderagen.idorder, orderagen.invoice, orderagen.harga,
                                                        orderagen.jumlah, orderagen.subtotal 
                                                        FROM orderagen 
                                                        inner join mitraagen on orderagen.idmitraagen=mitraagen.idmitraagen
                                                        inner join variants on orderagen.idproduk = variants.id
                        ...
                        <tr>
                         ...
                         <td>
                          ...
                          <td>
                            <?php if($_SESSION["administrator"]["nama"]=='Delita'){
                            echo "<td>";
    ...
    $sql = "SELECT * FROM orderagen 
            inner join variants on orderagen.idproduk = variants.id
            INNER JOIN products on products.id = variants.idproducts 
            WHERE orderagen.invoice='$invoice' and orderagen.jumlah>0 and products.idkategori>0 and products.idkategori<>2";
	$query = $koneksi->query($sql);
        ...
        ?>
                        ...
                        if(isset($_POST["done"])){
                          ...
                          <?php if(isset($_POST["hapus"])){
                                ...
                                $koneksi->query("DELETE FROM orderagen WHERE idorder='$id'" );
                                 $koneksi->query("UPDATE produk set stock=stock+'$jumlah' where idproduk='$idproduk' " );
                                 
                                echo "<script>alert('data berhasil dihapus');</script>";
		                        echo "<script>location='detailorderagen.php?invoice=$invoice';</script>";
                        }
		               ?>    
