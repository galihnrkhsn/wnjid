                      ...
                      <tbody>
                          ...
                          if(isset($_POST["cari"])){
                           ...
                           $mulai    =($page>1) ? ($page * $halaman) - $halaman : 0;
                            $datasaldo=$koneksi->query("SELECT podetail.variant, sum(pomaximus.jumlah) as jumlah FROM poproduk INNER JOIN pomaximus INNER JOIN pokategori INNER JOIN podetail on poproduk.idpoproduk=pomaximus.idpoproduk and pokategori.idpo=pomaximus.idpo and podetail.idpodetail=pomaximus.idpodetail where poproduk.namapo='$namapo' GROUP by podetail.variant");
                           ...
                           $page    =isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
                           $mulai    =($page>1) ? ($page * $halaman) - $halaman : 0;
                            $datasaldo=$koneksi->query("SELECT podetail.variant, sum(pomaximus.jumlah) as jumlah FROM poproduk INNER JOIN pomaximus INNER JOIN pokategori INNER JOIN podetail on poproduk.idpoproduk=pomaximus.idpoproduk and pokategori.idpo=pomaximus.idpo and podetail.idpodetail=pomaximus.idpodetail GROUP by podetail.variant");
                            $total = mysqli_num_rows($datasaldo);
                            $pages = ceil($total/$halaman);
