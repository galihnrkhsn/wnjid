...
<h3><strong>Input Keep Mitra</strong></h3>
...
<form method="post">
    ...
    <label>Exp</label><br>
    <input type="date" name="expire" class="form-control"><br>
    
    <label>Nama Mitra</label><br>
    <select name="kodemitra" class="form-control">
        <?php $ambil=$koneksi->query("SELECT * FROM admin_mitra");
        while($data=$ambil->fetch_assoc()){
        ?>
        <option value="<?php echo $data['kodemitra']; ?>"><?php echo $data['namamitra']; ?></option>
        <?php } ?>
    </select><br>
...
if(isset($_POST["save"])){
    ...
    $status=$_POST["status"];
		$koneksi->query("INSERT INTO keepproduk (namaproduk,qty,tgl,expire,kodemitra,statuspembayaran)
			VALUES ('$namaproduk','$qty','$tgl','$expire','$kodemitra','$status')");


			echo "<script>alert('data berhasil ditambah');</script>";
		echo "<script>location='index.php?page=inputkeepmitra';</script>";
