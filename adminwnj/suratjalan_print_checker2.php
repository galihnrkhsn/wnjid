<?php
    ...
    $datasj     = $koneksi->query("SELECT * FROM surat_jalan 
                                    WHERE no_sj = '$no_sj'");
...
<body>
    ...
    <table class="table" id="tb_multiprint" style="font-size: 25px; color: black;">
        ...
        <tbody>
            <?php
                if (substr($tampilsj['invoice'],0,1) == "F") {            
                    $datapo = $koneksi->query("SELECT surat_jalan.invoice, products.namaproduk, ordermitra.jumlah,
                                                    sum(surat_jalan.progres) as progres, surat_jalan.status,
                                                    surat_jalan.waktu, surat_jalan.id_sj, ordermitra.idmitra
                                                FROM surat_jalan
                                                ...
                                                WHERE surat_jalan.no_sj = '$no_sj' and surat_jalan.progres > 0
                                                GROUP BY variants.idproducts
                                            ");
                } else {
                    $datapo = $koneksi->query("SELECT surat_jalan.invoice, products.namaproduk, ordermitra.jumlah,
                                                    surat_jalan.progres, surat_jalan.status, surat_jalan.waktu,
                                                    surat_jalan.id_sj, ordermitra.idmitra, variants.variant, variants.size
                                                FROM surat_jalan
                                                JOIN variants on variants.id = surat_jalan.idproduk
                                                JOIN products on products.id = variants.idproducts
                                                JOIN ordermitra on ordermitra.idorder = surat_jalan.idorder
