<?php 
    ...
    header("Content-Disposition: attachment; filename=Penjualan Ready Stok DB.xls"); 
    session_start();
    include 'koneksi.php'; 
...
<body>
    ...
    <table class="table table-bordered table-striped mt-3" id="tbmaximus">
        ...
        <tbody>
            <?php 
                if ($filter == "Harian") {
                    $datapo = $koneksi->query("SELECT surat_jalan.invoice, SUM(surat_jalan.progres) as jumlah, surat_jalan.progres,
                                                    surat_jalan.status, surat_jalan.waktu, admin_mitra.namamitra, admin_mitra.idadmin, 
                                                    admin_mitra_cs.namacs, surat_jalan.id_sj, surat_jalan.no_sj, surat_jalan.invoice,
                                                    products.namaproduk, variants.harga, variants.variant, variants.size
                                                FROM surat_jalan
                                                ...
                                                ORDER BY admin_mitra.namamitra asc, surat_jalan.waktu DESC, surat_jalan.invoice");
                }
                if ($filter == "Bulanan") {
                    $datapo = $koneksi->query("SELECT surat_jalan.invoice, SUM(surat_jalan.progres) as jumlah, surat_jalan.progres,
                                                    surat_jalan.status, surat_jalan.waktu, admin_mitra.namamitra, admin_mitra.idadmin, 
                                                ...
                                                AND ordermitra.tgl > '2024-11-02'
                                                GROUP BY surat_jalan.id_sj
                                                ORDER BY admin_mitra.namamitra, surat_jalan.waktu DESC, surat_jalan.invoice");
                }
                $no = 1;
    ...
    <table class="table table-bordered table-striped mt-3" id="tbmaximus3">
        ...
        <tbody>
            <?php 
                if ($filter == "Harian") {
                    $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, SUM(surat_jalan_subdb.progres) as jumlah, surat_jalan_subdb.progres,
                                                    surat_jalan_subdb.status, surat_jalan_subdb.waktu, admin_mitra.namamitra, admin_mitra.idadmin, 
                                                    mitraagen.idmitraagen as idagen, mitrareseller.idmitrareseller as idreseller,
                                                    mitramarketer.idmitramarketer as idmarketer, mitraagen.namaagen as agen, 
                                                    mitrareseller.namaagen as reseller, mitramarketer.namaagen as marketer, admin_mitra_cs.namacs, 
                                                    surat_jalan_subdb.id_sj, surat_jalan_subdb.no_sj, products.namaproduk, variants.harga, 
                                                    variants.variant, variants.size
                                                FROM surat_jalan_subdb
                                                INNER JOIN variants on variants.id = surat_jalan_subdb.idproduk
                                                INNER JOIN products on products.id = variants.idproducts 
                                                ...
                                                ORDER BY admin_mitra.namamitra asc, surat_jalan_subdb.waktu DESC, surat_jalan_subdb.invoice");
                            }
                if ($filter == "Bulanan") {
                    $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, SUM(surat_jalan_subdb.progres) as jumlah,
                                                    ...
                                                    admin_mitra_cs.namacs, surat_jalan_subdb.id_sj, surat_jalan_subdb.no_sj, products.namaproduk, 
                                                    variants.harga, variants.variant, variants.size
                                                FROM surat_jalan_subdb
                                                INNER JOIN variants on variants.id = surat_jalan_subdb.idproduk 
                                                INNER JOIN products on products.id = variants.idproducts
                                                LEFT JOIN orderagen on orderagen.invoice = surat_jalan_subdb.invoice 
        ...
        <tbody>
            <?php 
                if ($filter=="Harian") {
                    $data_inv=$koneksi->query("SELECT surat_jalan_manual.invoice, SUM(surat_jalan_manual.progres) as jumlah,
                                                    ...
                                                    surat_jalan_manual.id_sj, products.namaproduk, variants.harga, 
                                                    variants.variant, variants.size
                                                ...
                                                ORDER BY admin_mitra.namamitra asc, surat_jalan_manual.waktu DESC, surat_jalan_manual.invoice");
                }
                if ($filter=="Bulanan") {
                    $data_inv=$koneksi->query("SELECT surat_jalan_manual.invoice, SUM(surat_jalan_manual.progres) as jumlah,
                                                    surat_jalan_manual.progres, surat_jalan_manual.status, surat_jalan_manual.waktu,
                                                    admin_mitra.namamitra, admin_mitra.idadmin, admin_mitra_cs.namacs, 
                                                    surat_jalan_manual.id_sj, products.namaproduk, variants.harga, variants.variant, 
                                                    variants.size
                                                FROM surat_jalan_manual
                                                JOIN admin_mitra on admin_mitra.idadmin = surat_jalan_manual.idadmin 
                                                INNER JOIN variants on variants.id = surat_jalan_manual.idproduk
            ...
            ?>
                <tr>
                    ...
                    <td><?= $tampilkan_inv['namaproduk']; ?> <?= $tampilkan_inv['variant']; ?> <?= $tampilkan_inv['size'] ?></td>
