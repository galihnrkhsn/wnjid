        ...
        if (isset($_POST['cetak'])) {
            try {
                ...
                if ($kode_mitra == "D") {
                    ...
                    $cid = $koneksi->query("SELECT iddb AS idmitra, namamitra AS namamitra FROM orderagen INNER JOIN admin_mitra ON admin_mitra.idadmin = orderagen.iddb WHERE invoice = '$invoice'");
                } elseif ($kode_mitra == "R") {
                    $cid = $koneksi->query("SELECT iddb AS idmitra, namamitra AS namamitra FROM orderreseller INNER JOIN admin_mitra ON admin_mitra.idadmin = orderreseller.iddb WHERE invoice = '$invoice'");
                } elseif ($kode_mitra == "M") {
                    $cid = $koneksi->query("SELECT iddb AS idmitra, namamitra AS namamitra FROM ordermarketer INNER JOIN admin_mitra ON admin_mitra.idadmin = ordermarketer.iddb WHERE invoice = '$invoice'");
                }
                $did = $cid->fetch_assoc();
                ...
                echo "
                    <script>
                        location='cetakpengiriman.php?id=$idorderp'
                    </script>
                ";
                

                // $query = $portal->query("INSERT INTO logistik3 VALUES
                //                             (
                //                                 NULL, '$today', '$penerima',
                //                                 '$ekspedisi', '', '$biayakirim',
                //                                 '$jumlah_koli', '$invoice', NULL,
                //                                 '$idadmin', '$namacs', '$no_sj',
                //                                 '$jenis_mitra'
                //                             )
                //                         ");
                // if ($query) {
                //     $clog = $portal->query("SELECT MAX(idlogistik) AS idlogistik FROM logistik");
                //     $dlog = $clog->fetch_assoc();
                //     $idlogistik = $dlog['idlogistik'];

                    // $query2 = $koneksi->query("INSERT INTO t_user VALUES
                    //                             (
                    //                                 NULL, '$idlogistik', '$namacs',
                    //                                 '$nama_pengirim', '$telepon_pengirim', '$penerima',
                    //                                 '$telepon_penerima', '$alamat', '$invoice',
