<?php 
    ...
    header("Content-Disposition: attachment; filename=Penjualan Ready Stok baru.xls");
    session_start();
    include 'koneksi.php';
        ...
        <tbody>
            <?php 
                if ($filter == "Harian") {
                    $datapo = $koneksi->query("SELECT surat_jalan_subdb.invoice, SUM(surat_jalan_subdb.progres) as jumlah,
                                                    ...
                                                    surat_jalan_subdb.waktu, admin_mitra.namamitra, admin_mitra.idadmin, 
                                                    ...
                                                    mitrareseller.namaagen as reseller, mitramarketer.namaagen as marketer,
                                                    admin_mitra_cs.namacs, surat_jalan_subdb.id_sj,
                                                    surat_jalan_subdb.no_sj, products.namaproduk, variants.harga, variants.variant, variants.size
                ...
                $no=1;
                while($tampilkan_subdb = $datapo->fetch_assoc()){
                    $result_explode = explode(' ', $tampilkan_subdb['waktu']);
                    $tanggal_rs_sub = $result_explode[0];
                    $no_sj          = $tampilkan_subdb['no_sj']; 
                    $invoice        = $tampilkan_subdb['invoice']; 
                    $totala         = $tampilkan_subdb['harga'];
                    $total_progres  = $tampilkan_subdb['progres'];
                    $diskon         = 35/100*$totala;
                    $totalnya       = $totala - $diskon;
            ...
            ?>
                <tr>
                    ...
                    <td><?= $tampilkan_subdb['namacs']; ?></td>
                    <td>
                        <?php 
                            if ($tampilkan_subdb['marketer']!='') {
                                echo $tampilkan_subdb['marketer'].'('.$tampilkan_subdb['idmarketer'].')'; 
                                $mitra = 'Marketer';              
                            }
                            elseif ($tampilkan_subdb['reseller']!='') {
                                echo $tampilkan_subdb['reseller'].'('.$tampilkan_subdb['idreseller'].')'; 
                                $mitra = 'Reseller'; 
                            }
                            elseif ($tampilkan_subdb['agen']!='') {
                                echo $tampilkan_subdb['agen'].'('.$tampilkan_subdb['idagen'].')';   
                                $mitra = 'Agen'; 
                            } 
                        ?>                   
                    </td>
                    <td><?= $tampilkan_subdb['no_sj']; ?></td>
                    <td><?= $tampilkan_subdb['invoice']; ?></td>  
                    <td><?= $tampilkan_subdb['namaproduk']; ?> <?= $tampilkan_subdb['variant']; ?> <?= $tampilkan_subdb['size']; ?></td>  
                    <td><?= $tampilkan_subdb['harga']; ?></td>  
                    <td><?= $totalnya; ?></td>            
                    <td><?= $total_progres; ?></td>
                    <td>Rp. <?= number_format($totalnya*$total_progres); ?></td>                 
                </tr>
            ...
            <?php 
                if ($filter == "Harian") {
                    $data_inv=$koneksi->query("SELECT surat_jalan_manual.invoice, SUM(surat_jalan_manual.progres) as jumlah,
                                                    ...
                                                    surat_jalan_manual.id_sj, products.namaproduk, variants.harga, variants.variant, variants.size
