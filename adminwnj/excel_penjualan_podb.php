<?php 
    ...
    header("Content-Disposition: attachment; filename=Penjualan PO per DB.xls");
    session_start();
    include 'koneksi.php'; 
...
<body>
    ...
    <table class="table table-bordered table-striped mt-3" id="tbmaximus">
        ...
        <tbody>
            <?php 
                if ($filter == "Harian") {
                    $datapo = $koneksi->query("SELECT surat_jalan_po.invoice, SUM(surat_jalan_po.progres) as jumlah,
                                                    surat_jalan_po.progres, surat_jalan_po.status, surat_jalan_po.waktu,
                                                    ...
                                                    mitramarketer.namaagen as marketer, admin_mitra_cs.namacs, surat_jalan_po.id_sj,
                                                    surat_jalan_po.no_sj, podetail.variant, podetail.harga
                                                FROM surat_jalan_po
                                                ...
                                                ORDER BY admin_mitra.namamitra asc, surat_jalan_po.waktu DESC, surat_jalan_po.idpodetail desc ");
                }
                if ($filter == "Bulanan") {
                    $datapo = $koneksi->query("SELECT surat_jalan_po.invoice, SUM(surat_jalan_po.progres) as jumlah,
                                                    surat_jalan_po.progres, surat_jalan_po.status, surat_jalan_po.waktu,
                                                    admin_mitra.namamitra, admin_mitra.idadmin, 
                                                    ...
                                                    admin_mitra_cs.namacs, surat_jalan_po.id_sj, surat_jalan_po.no_sj,
            ...
            <?php 
                if ($filter == "Harian") {
                    $data_inv = $koneksi->query("SELECT surat_jalan_manual.invoice, SUM(surat_jalan_manual.progres) as jumlah,
                                                        ...
                                                        surat_jalan_manual.id_sj, surat_jalan_manual.no_sj,
                                                        podetail.variant, podetail.harga
                                                    ...
                                                    ORDER BY admin_mitra.namamitra asc, surat_jalan_manual.waktu DESC, surat_jalan_manual.idproduk desc");
                }
                if ($filter == "Bulanan") {
                    $data_inv = $koneksi->query("SELECT surat_jalan_manual.invoice, SUM(surat_jalan_manual.progres) as jumlah,
                                                        surat_jalan_manual.progres, surat_jalan_manual.status,
                                                        surat_jalan_manual.waktu, admin_mitra.namamitra, 
                                                        admin_mitra.idadmin, admin_mitra_cs.namacs, surat_jalan_manual.id_sj,
                                                        surat_jalan_manual.no_sj, podetail.variant, podetail.harga
                                                    FROM surat_jalan_manual
                                                    JOIN podetail ON podetail.idpodetail = surat_jalan_manual.idproduk
                                                    LEFT JOIN admin_mitra on admin_mitra.idadmin = surat_jalan_manual.idadmin 
                                                    LEFT JOIN admin_mitra_cs on admin_mitra.idadmin = admin_mitra_cs.idadmin 
